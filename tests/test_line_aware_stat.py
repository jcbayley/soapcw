...
class TestCWLoad(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.las_py_approx = gen_lookup_python.LineAwareStatistic(
            self.arr,
            ndet=1,
            k=2,
            N=48,
            signal_prior_width=2,
            line_prior_width=3,
            noise_line_model_ratio=0.5,
            approx=True,
        )
        self.las_py_exact = gen_lookup_python.LineAwareStatistic(
            self.arr,
            ndet=1,
            k=2,
            N=48,
        ...
        if cpp_implementation:
            self.las_cpp_exact = gen_lookup.LineAwareStatistic(
                self.arr,
                ndet=1,
                k=2,
                N=48,
        ...
        self.las_py_approx_2det = gen_lookup_python.LineAwareStatistic(
            self.arr2,
            ndet=2,
            k=2,
            N=48,
        ...
        self.las_py_exact_2det = gen_lookup_python.LineAwareStatistic(
            self.arr2,
            ndet=2,
            k=2,
            N=48,
    ...
    def test_gen_lookup_1det_cpp_exact(self):
        ...
        if cpp_implementation:
            np.testing.assert_almost_equal(
                signoiseline, self.las_cpp_exact.signoiseline
            )
    ...
    def test_gen_lookup_2det_py_approx(self):
        ...
        signoiseline = np.array(
            [
                [0.28139104078142746, 0.3778010481689715],
                [0.3778010481689715, 0.6919634194611941],
            ]
        )

        np.testing.assert_almost_equal(
            signoiseline, self.las_py_approx_2det.signoiseline
        )
    ...
    def test_gen_lookup_2det_py_exact(self):
        ...
        signoiseline = np.array(
            [
                [0.27942796216954896, 0.37667858291387774],
                [0.37667858291387774, 0.6915352664549508],
            ]
        )

        np.testing.assert_almost_equal(
            signoiseline, self.las_py_exact_2det.signoiseline
        )
