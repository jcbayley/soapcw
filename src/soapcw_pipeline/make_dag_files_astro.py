...
def write_plot_subfile(sub_filename, config, config_file, cdirs, comment, verbose=True):
    with open(sub_filename, "w") as f:
        ...
        execute = os.path.join(
            os.path.split(sys.executable)[0], config["scripts"]["search_exec"]
        )
        f.write("executable = {}\n".format(execute))
        f.write("getenv  = True\n")
...
def write_dagfile(config_file, config, verbose=False):
    ...
    condor_dirs["save_directory"] = os.path.join(
        config["output"]["save_directory"],
        config["data"]["obs_run"],
    ...
    run_sub_comment = "soap_astro_F{}_{}_{}".format(
        bs1[0], be1[-1], config["data"]["obs_run"]
    )
    html_sub_comment = "soap_html_F{}_{}_{}".format(
        bs1[0], be1[-1], config["data"]["obs_run"]
    )
    ...
    for i, bs in enumerate(bs1):
        t_bss = np.arange(bs1[i], be1[i], config["condor"]["band_load_size"])[:-1]
        t_bes = (
            t_bss
            + config["condor"]["band_load_size"]
            + config["data"]["band_widths"][i]
        )
    ...
    with open(dag_filename, "w") as f:
        ...
        for i in range(len(band_starts)):
            ...
            vars_string = 'VARS {} bandstart="{}" bandend="{}"\n'.format(
                jobid, band_starts[i], band_ends[i]
            )
            f.write(job_string)
