...
def run_soap_onedet(config, degfree, sft, ind_start, ind_end):
    ...
    soaprunH1 = soap.single_detector(
        tr, sft.H1.downsamp_summed_norm_sft_power[:, ind_start:ind_end]
    )
    soaprunL1 = soap.single_detector(
        tr, sft.L1.downsamp_summed_norm_sft_power[:, ind_start:ind_end]
    )
...
def load_sft_band(config, filenames, minfreq, maxfreq, tmin, tmax, verbose=False):
    ...
    if "H1" not in sft.det_names:
        sft.H1 = cw.SFT()
        sft.H1.downsamp_summed_norm_sft_power = (
            np.ones(sft.L1.downsamp_summed_norm_sft_power.shape) * np.nan
        )
        sft.det_names = ["H1", "L1"]
        sft.H1.rng_med = sft.L1.rng_med * np.nan
    elif "L1" not in sft.det_names:
        sft.L1 = cw.SFT()
        sft.L1.downsamp_summed_norm_sft_power = (
            np.ones(sft.H1.downsamp_summed_norm_sft_power.shape) * np.nan
        )
        sft.det_names = ["H1", "L1"]
...
def plot_setup(
    ...
    (Hpwline,) = ax[4].plot(
        np.arange(sum_nsft),
        np.array(soap.tools.track_power(soaprunH1.vit_track, H_temp_sft)),
        color="C3",
        label="H1",
    )
    (Lpwline,) = ax[4].plot(
        np.arange(sum_nsft),
        np.array(soap.tools.track_power(soaprunL1.vit_track, L_temp_sft)),
        color="C2",
        label="L1",
    )
    ...
    (Hstatline,) = ax[5].plot(
        np.arange(sum_nsft),
        np.array(soap.tools.stat_on_path(soaprunH1.vit_track, soaprunH1.V)),
        color="C3",
        label="H1",
    )
    (Lstatline,) = ax[5].plot(
        np.arange(sum_nsft),
        np.array(soap.tools.stat_on_path(soaprunL1.vit_track, soaprunL1.V)),
        color="C2",
        label="L1",
    )
...
def run_soap_in_band(
    config, minfreq, maxfreq, weekstarttime=None, verbose=False, force_overwrite=False
):
    """_summary_
    ...
    if weekstarttime is None:
        ...
        if os.path.isdir(outpath_save):
            ...
            if force_overwrite == False:
                ...
                print(
                    f"WARNING!!! - Overwriting current existing directory: {outpath_save}"
                )
    ...
    # set output directories
    if weekstarttime is not None:
        outpath_save = os.path.join(
            outpath,
            config["output"]["sub_directory"],
            f"week_{startdatestring}_{enddatestring}",
        )
    else:
        outpath_save = os.path.join(outpath, config["output"]["sub_directory"])
    ...
    if os.path.isfile(tablefile):
        if not force_overwrite:
            raise FileExistsError(
                f"File {tablefile} already exists, add force-overwrite to arguments to overwrite file"
            )
    ...
    for num, start in enumerate(start_indicies):
        ...
        for det in getattr(sft, "det_names"):
            ...
            temp_noise[det] = av_noise(temp_n, nsft=config["data"]["n_summed_sfts"])[
                :sum_nsft
            ]
        ...
        with open(
            ...
            np.savetxt(
                f, np.transpose([track_times, tracks["H1"], tracks["L1"]]).astype(float)
            )
...
def main():
    ...
    parser.add_argument(
        "--force-overwrite",
        help="force overwrite tables and plots in output directory",
    ...
    run_soap_in_band(
        cfg, args.start_freq, args.end_freq, force_overwrite=args.force_overwrite
    )
