...
def load_sft_band(config, filenames, minfreq, maxfreq, tmin, tmax, verbose=False):
    ...
    # set how many frequency bins to average over for each frequency band
    # (based on spread of signal in 1800s sft)
...
def plot_setup(
    ...
    imvh1 = ax[2].imshow(
        soaprunH1.vitmap.T, cmap="cividis", aspect="auto", origin="lower", interpolation="none"
    )
    imvl1 = ax[3].imshow(
        soaprunL1.vitmap.T, cmap="cividis", aspect="auto", origin="lower", interpolation="none"
    )
...
def run_soap_in_band(
    ...
    if weekstarttime is not None:
        outpath_save = os.path.join(
            outpath, config["output"]["sub_directory"], f"week_{startdatestring}_{enddatestring}"
        )
    else:
        outpath_save = os.path.join(outpath, config["output"]["sub_directory"])
    ...
    if os.path.isfile(tablefile):
        if not force_overwrite:
            raise FileExistsError(
                f"File {tablefile} already exists, "
                f"add force-overwrite to arguments to overwrite file"
            )
    ...
    for num, start in enumerate(start_indicies):
        ...
        if num == 0:
            ...
            # for other plots just change data and axis labels
            # (runs faster that generating new figure)
...
def run(model, test_data, device="cpu"):
    ...
    with torch.no_grad():
        test_batch = torch.Tensor(np.array([val[0] for val in test_data])).to(device)
        test_labels = torch.Tensor(np.array([val[1] for val in test_data])).to(device)
...
def main():
    ...
    if args.config_file is not None:
        ...
        # outpath,minfreq,maxfreq,obs_run="O3",vitmapmodelfname=None,
        # spectmodelfname = None, vitmapstatmodelfname = None,
        # allmodelfname = None, sub_dir = "soap",
        cfg = {
            "output": {},
            "data": {},
            "input": {},
