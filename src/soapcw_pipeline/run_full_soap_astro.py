        ...
        if not os.path.isfile(fname) or force_overwrite:
            with open(fname, "w") as f:
                findout1 = subprocess.Popen(
                    ["find", sftpath, "-name", "*.sft"],
                    stdout=subprocess.PIPE,
                    shell=False,
                )
                findout = subprocess.run(
                    ["sort"], stdin=findout1.stdout, stdout=f, shell=False
                )
                print(findout)
        else:
...
def run_soap_twodet(config, degfree, sft, ind_start, ind_end):
    ...
    lookup_1 = os.path.join(
        ...
        f'log_signoiseline_1det_{degfree}degfree_{config["lookuptable"]["snr_width_line"]}_{config["lookuptable"]["snr_width_signal"]}_{config["lookuptable"]["prob_line"]}.pkl',
    )
    lookup_2 = os.path.join(
        config["lookuptable"]["lookup_dir"],
        f'log_signoiseline_2det_{degfree}degfree_{config["lookuptable"]["snr_width_line"]}_{config["lookuptable"]["snr_width_signal"]}_{config["lookuptable"]["prob_line"]}.pkl',
    )

    # define the transition matrix for soap
    tr = soap.tools.transition_matrix_2d(
...
def load_sft_band(config, filenames, minfreq, maxfreq, tmin, tmax, verbose=False):
    ...
    if verbose:
        print(
            "H1shape: {}, nsft: {}".format(np.shape(sft.H1.norm_sft_power), sft.H1.nsft)
        )
        print(
            "L1shape: {}, nsft: {}".format(np.shape(sft.L1.norm_sft_power), sft.L1.nsft)
        )

    sft.downsamp_frequency(
        stride=stride, data_type="summed_norm_sft_power", remove_original=False
    )
    ...
    if verbose:
        ...
        print(
            "H1shape sum: {}, freq: {}".format(
                np.shape(sft.H1.summed_norm_sft_power), minfreq
            )
        )
        print(
            "L1shape sum: {}, freq: {}".format(
                np.shape(sft.L1.summed_norm_sft_power), minfreq
            )
        )
        print(
...
def plot_setup(
    ...
    (Hpwline,) = ax[3].plot(
        np.arange(sum_nsft),
        np.array(soap.tools.track_power(soaprun.vit_track1, H_temp_sft)),
        color="C3",
        label="H1",
    )
    (Lpwline,) = ax[3].plot(
        np.arange(sum_nsft),
        np.array(soap.tools.track_power(soaprun.vit_track2, L_temp_sft)),
        color="C2",
        label="L1",
    )
    ...
    (Hstatline,) = ax[4].plot(
        np.arange(sum_nsft),
        np.array(soap.tools.stat_on_path(soaprun.vit_track, soaprun.V)),
        color="C3",
        label="H1",
    )
    ...
    # plot noise
    (Hnoise,) = ax[5].plot(
        np.arange(sum_nsft), temp_noise["H1"], color="C3", label="H1"
    )
    (Lnoise,) = ax[5].plot(
        np.arange(sum_nsft), temp_noise["L1"], color="C2", label="L1"
    )
...
def update_plot(
    ...
    Hpwline.set_ydata(soap.tools.track_power(soaprun.vit_track1, H_temp_sft))
    Lpwline.set_ydata(soap.tools.track_power(soaprun.vit_track2, L_temp_sft))
    ax[3].relim()
    ax[3].autoscale_view()

    # set the new data and limits for the vitstat plot
    Hstatline.set_ydata(soap.tools.stat_on_path(soaprun.vit_track, soaprun.V))
    ax[4].relim()
    ax[4].autoscale_view()
...
def run_soap_in_band(config, minfreq, maxfreq, verbose=False):
    """_summary_
    ...
    outpath = os.path.join(
        *[config["output"]["save_directory"], config["data"]["obs_run"]]
    )
    ...
    sft_filelists = get_sft_files_find(
        outpath, sftfiles, config["output"]["overwrite_files"]
    )
    ...
    start_indicies = np.round(np.arange(0, nbins, width / (2 * bin_width))).astype(int)[
        :-1
    ]
    ...
    for num, start in enumerate(start_indicies):
        ...
        for det in getattr(sft, "det_names"):
            ...
            temp_n = np.sqrt(
                np.nanmedian(getattr(sft, det).rng_med[:, ind_start:ind_end], axis=1)
            )
        ...
        save_path = os.path.join(
            outpath_save_plots,
            "notrack_F{}_{}.png".format(np.round(fmin, 2), np.round(fmax, 2)),
        )
        save_path_track = os.path.join(
            outpath_save_plots,
            "track_F{}_{}.png".format(np.round(fmin, 2), np.round(fmax, 2)),
        )
        ...
        if config["cnn"]["vitmapmodel_path"] is not None:
            vitmap_cnn_prob = run_vitmap_cnn(
                os.path.join(
                    config["cnn"]["vitmapmodel_path"],
                    "{}_model.pt".format(train_band_type),
                ),
                soaprun.vitmap,
            )
        ...
        if config["cnn"]["vitmapstatmodel_path"] is not None:
            vitmapstat_cnn_prob = run_vitmapstat_cnn(
                os.path.join(
                    config["cnn"]["vitmapstatmodel_path"],
                    "{}_model.pt".format(train_band_type),
                ),
                soaprun.vitmap,
        ...
        with open(
            os.path.join(
                outpath_save_track,
                "track_F{}_{}.txt".format(np.round(fmin, 2), np.round(fmax, 2)),
            ),
            "w",
        ) as f:
            np.savetxt(
                f,
                np.transpose(
                    [track_times, tracks["H1"], tracks["L1"], tracks["SSB"]]
                ).astype(float),
            )
        ...
        for det in getattr(sft, "det_names"):
            temp_noise[det][
                sft.H1.downsamp_summed_norm_sft_power[:, 0] == degfree
            ] = np.nan
        ...
        xticktimes = [
            Time(tstart + i * 48 * tsft, format="gps").datetime for i in xticks
        ]
        xticklabels = [
            "{}-{}".format(tm.day, months[tm.month - 1]) for tm in xticktimes
        ]
...
def run(model, test_data, device="cpu"):
    ...
    with torch.no_grad():
        test_batch, test_labels = torch.Tensor(
            np.array([val[0] for val in test_data])
        ).to(device), torch.Tensor(np.array([val[1] for val in test_data])).to(device)
...
def run_all_cnn(
    modelfname, vit_data, H_data, L_data, vit_stat, size=(156, 89), degfree=96
):
    """
    run the cnn
    """
...
def main():
    ...
    parser.add_argument(
        "-i", "--sft-path", help="path to sfts", type=str, required=False
    )
    parser.add_argument(
        "-s",
        "--start-freq",
        help="start frequency for band",
        type=float,
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-r", "--obs-run", help="observing run", type=str, required=False
    )
    parser.add_argument(
        "-l",
        "--lookup-dir",
        help="path to soap lookup tables",
        type=str,
        required=False,
    )
    parser.add_argument(
    ...
    run_soap_in_band(cfg, args.start_freq, args.end_freq)
