...
def plot_single(spect, soapout=None, fmin=None, fmax=None, tsft=None, tstart=None):
    ...
    plot_spect = np.ma.masked_where(spect == 96.0, spect, copy=True)

    im = ax[0].imshow(plot_spect.T, cmap="YlGnBu", aspect="auto", origin="lower")
    if soapout is not None:
        imv = ax[1].imshow(
            soapout.vitmap.T, cmap="YlGnBu", aspect="auto", origin="lower"
        )
    ...
    if soapout is not None:
        line = ax[0].scatter(
            np.arange(nsft), soapout.vit_track, color="red", label="Viterbi track", s=3
        )
