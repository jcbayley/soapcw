...
class LineAwareStatistic:
    def __init__(
        ...
        k=2,
        N=48,
        signal_prior_width=1,
        line_prior_width=5,
        noise_line_model_ratio=1,
        approx=True,
    ):
        self.ndet = ndet
        ...
        if ndet == 1:
            (
                ...
                self.noise,
                self.line,
            ) = self.gen_lookup_one_det(
                powers,
                approx=approx,
                signal_prior_width=signal_prior_width,
                line_prior_width=line_prior_width,
                noise_line_model_ratio=noise_line_model_ratio,
            )
        elif ndet == 2:
            (
                self.signoiseline,
    ...
    def chi2_sig(self, lamb: np.array, gs: float, pv: float) -> np.array:
        ...
        if len(np.shape(gs)) == 0:
            ...
            func = np.prod(
                [st.ncx2.pdf(i, df=self.kN, nc=lamb, loc=0, scale=1) for i in gs],
                axis=0,
            )
        wid = 1.0 / pv
    ...
    def chi2_line(self, lamb, gs, pv):
        ...
        if len(np.shape(gs)) == 0:
            ...
            func = (
                ...
                / len(gs)
                * (
                    st.chi2.pdf(gs[0], df=self.kN, loc=0, scale=1)
                    * st.ncx2.pdf(gs[1], df=self.kN, nc=lamb, loc=0, scale=1)
                    + st.ncx2.pdf(gs[0], df=self.kN, nc=lamb, loc=0, scale=1)
                    * st.chi2.pdf(gs[1], df=self.kN, loc=0, scale=1)
                )
            )
        wid = 1.0 / pv
        return func * wid * np.exp(-wid * lamb)
    ...
    def chi2_noise(self, gs):
        if len(np.shape(gs)) == 0:
            ...
            func = np.prod(
                [st.chi2.pdf(i, df=self.kN, loc=0, scale=1) for i in gs], axis=0
            )
        return func
    ...
    def two_det(
        ...
        g1,
        g2,
        signal_prior_width=10,
        line_prior_width=10,
        noise_line_model_ratio=1.0,
        approx=True,
    ):
        """
        ...
        if approx:
            ...
            sig_int = np.trapz(
                np.nan_to_num(self.chi2_sig(l, [g1, g2], signal_prior_width)), l
            )
            line_int = np.trapz(
                np.nan_to_num(self.chi2_line(l, [g1, g2], line_prior_width)), l
            )
        else:
            sig_int, sig_err = quad(
                self.chi2_sig, 0, np.inf, args=([g1, g2], signal_prior_width)
            )
            line_int, line_err = quad(
                self.chi2_line, 0, np.inf, args=([g1, g2], line_prior_width)
            )
    ...
    def one_det(
        ...
        if approx:
            l = np.linspace(0, 100, 500)
            sig_int = np.trapz(
                np.nan_to_num(self.chi2_sig(l, g1, signal_prior_width)), l
            )
            line_int = np.trapz(
                np.nan_to_num(self.chi2_line(l, g1, line_prior_width)), l
            )
        else:
            sig_int, sig_err = quad(
                self.chi2_sig, 0, np.inf, args=(g1, signal_prior_width)
            )
            line_int, line_err = quad(
                self.chi2_line, 0, np.inf, args=(g1, line_prior_width)
            )
    ...
    def save_lookup(self, outdir, log=True, stat_type="signoiseline"):
        ...
        fname = os.path.join(
            outdir,
            "{}{}_{}det_{}degfree_{}_{}_{}.pkl".format(
                log_str,
                stat_type,
                self.ndet,
                self.kN,
                self.signal_prior_width,
                self.line_prior_width,
                self.noise_line_model_ratio,
            ),
        )
        ...
        if os.path.isfile(fname):
            ...
            with open(fname, "wb") as f:
                header = {
                    "power_ranges": (minimum, maximum, num),
                    "fraction_ranges": (1, 1, 1),
                    "signal_prior": self.signal_prior_width,
                    "line_prior_width": self.line_prior_width,
                    "noise_line_model_ratio": self.noise_line_model_ratio,
                }
                if log:
    ...
    def save_multiple(
        self,
        powerrange,
        signal_prior_widthrange,
        line_prior_widthrange,
        noise_line_model_ratiorange,
    ):
        pass
...
class LineAwareAmpStatistic:
    ...
    def chi2_sig(self, lamb, gs, pv, fraction):
        ...
        if len(np.shape(gs)) == 0:
            ...
            func = np.prod(
                [
                    st.ncx2.pdf(gs[i], df=self.kN, nc=lamb_list[i], loc=0, scale=1)
            ...
            if fraction > 1:
                func = (
                    ...
                    * (
                        st.chi2.pdf(gs[0], df=self.kN, loc=0, scale=1)
                        * st.ncx2.pdf(
                            gs[1], df=self.kN, nc=lamb * fraction, loc=0, scale=1
                        )
                        + st.ncx2.pdf(gs[0], df=self.kN, nc=lamb, loc=0, scale=1)
                        * st.chi2.pdf(gs[1], df=self.kN, loc=0, scale=1)
                    )
                ...
                func = (
                    ...
                    * (
                        ...
                        * st.ncx2.pdf(gs[1], df=self.kN, nc=lamb, loc=0, scale=1)
                        + st.ncx2.pdf(
                            gs[0], df=self.kN, nc=lamb * fraction, loc=0, scale=1
                        )
                        * st.chi2.pdf(gs[1], df=self.kN, loc=0, scale=1)
                    )
                )
    ...
    def two_det(
        ...
        if approx:
            l = np.linspace(0, 100, 500)
            sig_int = np.trapz(
                np.nan_to_num(self.chi2_sig(l, [g1, g2], fraction, signal_prior_width)),
                l,
            )
            line_int = np.trapz(
                np.nan_to_num(self.chi2_line(l, [g1, g2], fraction, line_prior_width)),
                l,
            )
        else:
            sig_int, sig_err = quad(
                self.chi2_sig, 0, np.inf, args=([g1, g2], fraction, signal_prior_width)
            )
            line_int, line_err = quad(
                self.chi2_line, 0, np.inf, args=([g1, g2], fraction, line_prior_width)
            )
    ...
    def save_lookup(self, outdir, log=True, stat_type="signoiseline"):
        ...
        fname = os.path.join(
            outdir,
            "{}{}amp_{}det_{}degfree_{}_{}_{}.pkl".format(
                log_str,
                stat_type,
                self.ndet,
                self.kN,
        ...
        if os.path.isfile(fname):
            ...
            with open(fname, "wb") as f:
                header = {
                    "power_ranges": (minimum, maximum, num),
                    "fraction_ranges": (minimumf, maximumf, numf),
                    "signal_prior": self.signal_prior_width,
                    "line_prior_width": self.line_prior_width,
                    "noise_line_model_ratio": self.noise_line_model_ratio,
                }
...
class LineAwareAmpStatistic_old:
    def __init__(
        ...
        if ndet == 1:
            ...
            (
                ...
                fractions,
                approx=approx,
                signal_prior_width=self.signal_prior_width,
                line_prior_width=self.line_prior_width,
                noise_line_model_ratio=self.noise_line_model_ratio,
            )
    ...
    def chi2_sig(self, lamb, g1, g2, pv, fraction):
        ...
        """
        if fraction >= 1:
            func = st.ncx2.pdf(g1, df=self.kN, nc=lamb, loc=0, scale=1) * st.ncx2.pdf(
                g2, df=self.kN, nc=lamb * fraction, loc=0, scale=1
            )
        else:
            func = st.ncx2.pdf(
                g1, df=self.kN, nc=lamb / fraction, loc=0, scale=1
            ) * st.ncx2.pdf(g2, df=self.kN, nc=lamb, loc=0, scale=1)
    ...
    def chi2_line(self, lamb, g1, g2, pv, fraction):
        ...
        if fraction >= 1:
            func = (
                ch2_v[0]
                * st.ncx2.pdf(g2, df=self.kN, nc=lamb * fraction, loc=0, scale=1)
                + st.ncx2.pdf(g1, df=self.kN, nc=lamb, loc=0, scale=1) * ch2_v[1]
            )
        else:
            func = (
                ch2_v[0] * st.ncx2.pdf(g2, df=self.kN, nc=lamb, loc=0, scale=1)
                + st.ncx2.pdf(g1, df=self.kN, nc=lamb / fraction, loc=0, scale=1)
                * ch2_v[1]
            )
    ...
    def line_aware_amp(
        ...
        noise = self.chi2_noise(g1, g2)
        sig_int, sig_err = quad(
            self.chi2_sig, 0, np.inf, args=(g1, g2, signal_prior_width, fraction)
        )
        line_int, line_err = quad(
            self.chi2_line, 0, np.inf, args=(g1, g2, line_prior_width, fraction)
        )
    ...
    def line_aware_amp_approx(
        ...
        noise = self.chi2_noise(g1, g2)
        sig_int = np.trapz(
            np.nan_to_num(self.chi2_sig(l, g1, g2, signal_prior_width, fraction)), l
        )
        line_int = np.trapz(
            np.nan_to_num(self.chi2_line(l, g1, g2, line_prior_width, fraction)), l
        )
    ...
    def line_aware_amp_cpp(
        ...
        """
        sig_int = integrals.Integral(
            [g1], g2, logfrac, self.k, self.N, signal_prior_width, "signal"
        )
        line_int = integrals.Integral(
            [g1], g2, logfrac, self.k, self.N, line_prior_width, "line"
        )
        bsgl1 = sig_int / (
            noise_line_model_ratio * line_int + chi2_noise(g1, g2, self.k, self.N)
        )
    ...
    def gen_lookup_two_det(
        ...
        for i in range(len(powers)):
            for j in range(len(powers)):
                ...
                for k in range(len(fractions)):
                    if approx:
                        las = self.line_aware_amp_approx(
                            powers[i],
                            powers[j],
                            fractions[k],
                            signal_prior_width,
    ...
    def save_lookup(self, outdir, stat_type="signoiseline", log=True):
        ...
        """
        minimum, maximum, num = min(self.powers), max(self.powers), len(self.powers)
        minimum_frac, maximum_frac, num_frac = (
            min(self.fractions),
            max(self.fractions),
            len(self.fractions),
        )
        ...
        if os.path.isfile(fname):
            ...
            with open(fname, "wb") as f:
                header = {
                    "power_ranges": (minimum, maximum, num),
                    "fraction_ranges": (minimum_frac, maximum_frac, num_frac),
                    "signal_prior": self.signal_prior_width,
                    "line_prior_width": self.line_prior_width,
                    "noise_line_model_ratio": self.noise_line_model_ratio,
                }
