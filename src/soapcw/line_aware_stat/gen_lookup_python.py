...
class LineAwareStatistic:
    ...
    def chi2_sig(self, lamb: np.array, gs: float, pv: float) -> np.array:
        ...
        if len(np.shape(gs)) == 0:
            ...
            func = np.prod(
                [st.ncx2.pdf(i, df=self.kN, nc=lamb, loc=0, scale=1) for i in gs], axis=0
            )
        wid = 1.0 / pv
...
class LineAwareAmpStatistic:
        ...
        if len(np.shape(gs)) == 0:
            ...
            if fraction > 1:
                func = (
                    ...
                    * (
                        st.chi2.pdf(gs[0], df=self.kN, loc=0, scale=1)
                        * st.ncx2.pdf(gs[1], df=self.kN, nc=lamb * fraction, loc=0, scale=1)
                        + st.ncx2.pdf(gs[0], df=self.kN, nc=lamb, loc=0, scale=1)
                        * st.chi2.pdf(gs[1], df=self.kN, loc=0, scale=1)
                    )
                )
                ...
                func = (
                    ...
                    * (
                        st.chi2.pdf(gs[0], df=self.kN, loc=0, scale=1)
                        * st.ncx2.pdf(gs[1], df=self.kN, nc=lamb, loc=0, scale=1)
                        + st.ncx2.pdf(gs[0], df=self.kN, nc=lamb * fraction, loc=0, scale=1)
                        * st.chi2.pdf(gs[1], df=self.kN, loc=0, scale=1)
                    )
                )
    ...
    def two_det(
        ...
        if approx:
            l = np.linspace(0, 100, 500)
            sig_int = np.trapz(
                np.nan_to_num(self.chi2_sig(l, [g1, g2], fraction, signal_prior_width)), l
            )
            line_int = np.trapz(
                np.nan_to_num(self.chi2_line(l, [g1, g2], fraction, line_prior_width)), l
            )
        else:
...
class LineAwareAmpStatistic_old:
    ...
    def chi2_sig(self, lamb, g1, g2, pv, fraction):
        ...
        if fraction >= 1:
            ...
            func = st.ncx2.pdf(g1, df=self.kN, nc=lamb / fraction, loc=0, scale=1) * st.ncx2.pdf(
                g2, df=self.kN, nc=lamb, loc=0, scale=1
            )
    ...
    def chi2_line(self, lamb, g1, g2, pv, fraction):
        ...
        if fraction >= 1:
            func = (
                ch2_v[0] * st.ncx2.pdf(g2, df=self.kN, nc=lamb * fraction, loc=0, scale=1)
                + st.ncx2.pdf(g1, df=self.kN, nc=lamb, loc=0, scale=1) * ch2_v[1]
            )
        else:
            func = (
                ch2_v[0] * st.ncx2.pdf(g2, df=self.kN, nc=lamb, loc=0, scale=1)
                + st.ncx2.pdf(g1, df=self.kN, nc=lamb / fraction, loc=0, scale=1) * ch2_v[1]
            )
    ...
    def line_aware_amp_cpp(
        self, g1, g2, logfrac, signal_prior_width=1, line_prior_width=1, noise_line_model_ratio=1.0
    ):
        """
        marginalises over the "SNR" for both the noise and line case and combines into an odds noise_line_model_ratio (approximate using trapz)
        args
    ...
    def gen_lookup_cpp(
        self, g1, g2, logfrac, signal_prior_width, line_prior_width, noise_line_model_ratio
    ):
        """
        calculates the odds noise_line_model_ratio in ch_integrals_approx_noise for given values of power and noise_line_model_ratio.
        args
