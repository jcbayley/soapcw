...
def save_lookup_amp(
    p1, p2, ratio, outdir, ndet=2, pow_range=(1, 400, 500), frac_range=(0.1, 1, 10)
):
    """
    ...
    filename = outdir + "/ch2_signoiseline_{}det_{}_{}_{}.json".format(
        ndet, p1, p2, ratio
    )
    if os.path.isfile(filename):
        ...
        ch_arr_app = gen_lookup.LineAwareAmpStatistic(
            ...
            fraction=np.linspace(minn, maxn, numn),
            ndet=ndet,
        ...
        with open(filename, "w+") as f:
            save_data = [
                [minimum, maximum, num, minn, maxn, numn],
                np.log(ch_arr_app[0]).tolist(),
            ]
...
def save_lookup(p1, p2, ratio, outdir, ndet=2, pow_range=(1, 400, 500), k=2, N=48):
    ...
    if os.path.isfile(
        outdir + "/signoiseline_{}det_{}_{}_{}.txt".format(ndet, p1, p2, ratio)
    ):
        pass
    else:
        if ndet == 1:
            ch_arr_app = gen_lookup.LineAwareStatistic(
                np.linspace(minimum, maximum, num),
                ndet=ndet,
                signal_prior_width=p1,
                line_prior_width=p2,
...
def resave_files(p1, p2, ratio, output):
    ...
    """
    if os.path.isfile(
        output + "/txt/ch2_signoiseline_{}_{}_{}.txt".format(p1, p2, ratio)
    ):
        with open(
            output + "/txt/ch2_signoiseline_{}_{}_{}.txt".format(p1, p2, ratio), "rb"
        ) as f:
        ...
        if os.path.isdir(output + "/pkl/"):
            ...
            os.mkdir(output + "/pkl/")
        with open(
            output + "/pkl/ch2_signoiseline_{}_{}_{}.pkl".format(p1, p2, ratio), "wb"
        ) as f:
