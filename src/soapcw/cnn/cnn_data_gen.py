...
def find_sft_file(indir, bandmin, bandmax):
    ...
    if hname is None or lname is None:
        print(
            "NO SFT FOUND {}, {}, {}, F{:0>4}Hz".format(
                indir, bandmin, bandmax, int(bandmin)
            )
        )
...
def return_outputs(
    ...
    lookup_1 = str(lookup_dir) + "/log_signoiseline_1det_{}degfree_{}_{}_{}.pkl".format(
        degfree, 4.0, 10.0, 0.4
    )
    lookup_2 = str(lookup_dir) + "/log_signoiseline_2det_{}degfree_{}_{}_{}.pkl".format(
        degfree, 4.0, 10.0, 0.4
    )
    ...
    if inj_track is not None and "diffs" in save_options:
        diffs = (
            np.array(viterbi.vit_track1)
            - np.array(inj_track)[: len(viterbi.vit_track1)]
        )
    ...
    if "powers" in save_options:
        powers = soap.tools.track_power(
            viterbi.vit_track1, datah
        ), soap.tools.track_power(viterbi.vit_track2, datal)
...
def main():
    if len(sys.argv) < 2 or sys.argv[1] == "-h":
        ...
        parser.add_argument(
            "-v", "--verbose", help="display status", action="store_true"
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "-dt",
            "--data-type",
            help="train, validation or test data",
            type=str,
            default="train",
        )
        parser.add_argument(
