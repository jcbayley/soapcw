...
# import mdc_test
from soapcw.cnn import generate_gaussian_train_data, generate_test_data, generate_train_data
...
def return_outputs(
    ...
    if "stats" in save_options:
        ...
        if config["lookuptable"]["lookup_type"] == "power":
            ...
            viterbi = soapcw.two_detector(
                tr, datah, datal, lookup_table_2det=lookup_2, lookup_table_1det=lookup_1
            )
        else:
    ...
    if "powers" in save_options:
        powers = soapcw.tools.track_power(viterbi.vit_track1, datah), soapcw.tools.track_power(
            viterbi.vit_track2, datal
        )
    ...
    if "plots" in save_options:
        ...
        plotfig = make_figure(
            ...
            datah,
            datal,
            [viterbi.vit_track1, viterbi.vit_track2],
            powers,
            vit_data,
            1800.0,
            plot_track=True,
    ...
    if np.shape(vit_resize_norm) != np.shape(deth_resize_norm) or np.shape(
        vit_resize_norm
    ) != np.shape(detl_resize_norm):
        print("Wrong Sizes")
        sys.exit()
...
def make_figure(
    flow, fhigh, tstart, tend, sum_obs1, sum_obs2, tracks, powers, vit_m, tsft, plot_track=True
):
    ...
    if plot_track:
        ax[0].plot(
            np.linspace(tstart, tend, len(tracks[0])),
            tracks[0] / tsft + flow,
            "C0",
            label="Viterbi track",
        )
        ax[1].plot(
            np.linspace(tstart, tend, len(tracks[1])),
            tracks[1] / tsft + flow,
            "C2",
            label="Viterbi track",
        )
...
def main():
    if len(sys.argv) < 2 or sys.argv[1] == "-h":
        ...
        parser.add_argument(
            "-l", "--band-min", help="lower frequency of band", type=float, required=True
        )
        parser.add_argument(
            "-u", "--band-max", help="upper frequency of band", type=float, required=True
        )
        parser.add_argument(
            "-w", "--band-width", help="width of indidual band", type=float, required=True
        )
        parser.add_argument(
            "-dt", "--data-type", help="train, validation or test data", type=str, default="train"
        )
        parser.add_argument(
            "-rt",
            "--run-type",
            help="run type, gauss: return gaussian noise, else:O1,O2,O3,(O4)",
            required=True,
        )
        parser.add_argument(
            "-sr",
            "--search",
            help="generate search data, if true generates data with no injections",
            type=str2bool,
            const=True,
            default=False,
            nargs="?",
        ...
        parser.add_argument(
            "-nn",
            "--no-noise",
            help="do not regenerate noise bands",
            type=str2bool,
            const=True,
            default=False,
            nargs="?",
        ...
        parser.add_argument(
            ...
            help="files to save",
            type=str,
            default="vit_img stats pars H_imgs L_imgs vit_imgs",
            nargs="?",
        )
        parser.add_argument(
            "-np",
            "--nperband",
            help="numnber of examples per band gaussian noise",
            type=int,
            default=20,
            nargs="?",
        )
        ...
        if args.data_type == "test":
            ...
            if args.run_type != "gaussian":
                generate_test_data.loop_band_test_data(
                    args.load_dir,
                    save_dir,
                    args.band_min,
                    args.band_max,
                    args.band_width,
                    args.resize_image,
                    num_repeat=args.nperband,
                )
            elif args.run_type == "gaussian":
                gaussian_train.loop_band_train_augment(
                    loadpath=args.load_dir,
                    path=save_dir,
                    ...
                    nperband=1,
                    test=True,
                    save_options=args.save_options.split(" "),
                )
