...
class LoadDataOld(torch.utils.data.Dataset):
    def __init__(
        self,
        noise_load_directory,
        signal_load_directory,
        load_types=["stats", "vit_imgs", "H_imgs", "L_imgs"],
        shuffle=True,
        nfile_load="all",
        snr_min=None,
        snr_max=None,
        ...
        if (
            "H_imgs" in self.load_types
            and "L_imgs" in self.load_types
            and "vit_imgs" in self.load_types
        ):
            self.n_load_types = len(self.load_types) - 2
        elif (
            "H_imgs" in self.load_types
            and "L_imgs" in self.load_types
            and "vit_imgs" not in self.load_types
        ):
    ...
    def old___getitem__(self, idx):
        ...
        tot_data = [
            torch.cat(
                [torch.Tensor(noise_data[i]).squeeze(), torch.Tensor(signal_data[i]).squeeze()],
                dim=0,
            )
            for i in range(self.n_load_types)
        ]
    ...
    def __getitem__(self, idx):
        ...
        all_data, all_truths = [
            self.all_data[i][sub_index : sub_index + self.batch_size]
            for i in range(self.n_load_types)
        ], self.all_truths[sub_index : sub_index + self.batch_size]
    ...
    def load_files_to_data(self):
        ...
        for i in range(n_load_files):
            noise_data, noise_pars, pname = self.load_file(
                self.noise_filenames[i + self.start_file_ind], noise_only=True
            )
            signal_data, signal_pars, pname = self.load_file(
                self.signal_filenames[i + self.start_file_ind]
            )
            pars = list(noise_pars) + list(signal_pars)
        ...
        self.all_data = [
            torch.cat([all_data[i][j] for i in range(n_load_files)], dim=0)
            for j in range(self.n_load_types)
        ]
    ...
    def load_file(self, fname, noise_only=False):
                    ...
                    if imgdone:
                        ...
                        output_data.append(
                            np.transpose(
                                np.concatenate(
                                    [
                                        np.expand_dims(f["H_imgs"], -1),
                                        np.expand_dims(f["L_imgs"], -1),
                                        np.expand_dims(f["vit_imgs"], -1),
                                    ],
                                    axis=-1,
                                ),
                                (0, 3, 2, 1),
        ...
        if self.snr_min is not None and self.snr_max is not None and noise_only is False:
            ...
            output_data = [
                output_data[i][np.logical_and(snrs >= self.snr_min, snrs <= self.snr_max)]
    ...
    def get_filenames(self):
        self.noise_filenames = [
            os.path.join(self.noise_load_directory, fname)
            for fname in os.listdir(self.noise_load_directory)
        ][:4]
        self.signal_filenames = [
            os.path.join(self.signal_load_directory, fname)
            for fname in os.listdir(self.signal_load_directory)
        ][:4]
...
class LoadData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        all_data, all_truths = [
            self.all_data[i][idx] for i in range(self.n_load_types)
        ], self.all_truths[idx]
    ...
    def load_files_to_data(self):
        ...
        self.all_data = [
            torch.cat([all_data[i][j] for i in range(len(self.noise_filenames))], dim=0)
            for j in range(self.n_load_types)
        ]
        self.all_truths = torch.cat(all_truths, dim=0)
    ...
    def load_file(self, fname, noise_only=False):
        ...
        if self.hardware_injections is not None:
            for _, hinj in self.hardware_injections.iterrows():
                ...
                inband = np.logical_and(
                    pars[:, parnames.index("fmin")] <= hinj["f0 (epoch start)"],
                    pars[:, parnames.index("fmax")] >= hinj["f0 (epoch start)"],
                )
                pars = pars[~inband]
    ...
    def get_filenames(self):
        ...
        if self.shuffle:
            ...
            self.noise_filenames = sorted(
                self.noise_filenames, key=lambda a: float(a.split("_")[-3])
            )
            self.signal_filenames = sorted(
                self.signal_filenames, key=lambda a: float(a.split("_")[-3])
            )
        else:
            pass
...
def train_multi_batch(
    ...
    for i in range(n_train_multi_size):
        ...
        for sind in start_inds:
            if model_type in ["spectrogram", "vitmapspectrogram"]:
                output = model(
                    torch.Tensor(batch_data[0][:, :, :, sind : sind + seglen]).to(device)
                )
...
def train_model(
    ...
    train_noise_dir = os.path.join(
        load_dir, "train", bandtype, f"band_{fmin:.1f}_{fmax:.1f}", "snr_0.0_0.0"
    )
    train_signal_dir = os.path.join(
        load_dir,
        "train",
        bandtype,
        f"band_{fmin:.1f}_{fmax:.1f}",
        f"snr_{float(snrmin):.1f}_{float(snrmax):.1f}",
    )

    val_noise_dir = os.path.join(
        load_dir, "train", other_bandtype, f"band_{fmin:.1f}_{fmax:.1f}", "snr_0.0_0.0"
    )
    val_signal_dir = os.path.join(
        load_dir,
        "train",
        other_bandtype,
        f"band_{fmin:.1f}_{fmax:.1f}",
    ...
    model_fname = os.path.join(
        save_dir, f"model_{model_type}_for_{other_bandtype}_F{fmin}_{fmax}.pt"
    )

    if os.path.isfile(model_fname) and not overwrite and not continue_train:
        raise Exception(
            f"Model file {model_fname} already exists, set overwrite=True to overwrite or continue_train=True to continue training"
        )
    ...
    if model_type == "spectrogram":
        ...
        raise Exception(
            f"Load type {model_type} not defined select from [spectrogram, vitmap, vit_imgs, vitmapspectrogram, vitmapspectstatgram]"
        )
    ...
    validation_dataset = torch.utils.data.DataLoader(
        validation_dataset_1, batch_size=64, shuffle=False
    )
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimiser, eta_min=scheduler_weight * learning_rate, T_max=scheduler_length
    )
    ...
    if load_model is not None:
        if os.path.isfile(load_model):
            ...
            load_model_fname = os.path.join(
                load_model, f"model_{model_type}_for_{other_bandtype}_F{fmin}_{fmax}.pt"
            )
        else:
        ...
        if continue_train:
            ...
            with open(
                os.path.join(
                    save_dir, f"losses_for_{model_type}_{other_bandtype}_F{fmin}_{fmax}.txt"
                ),
                "r",
            ) as f:
    ...
    if n_train_multi_size not in [None, "none"] and avg_pool_size in [None, "none"]:
        raise Exception(
            "Train multi batch can only be used with adaptive average pooling avg_pool_size"
        )
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for batch_data, batch_labels, batch_pars in train_dataset:
            ...
            if verbose:
                print(
                    f"Batch {batch_number}, mean_batch_time: {np.mean(mean_batch_time[1:])}, loss: {loss}"
                )
            for _ in range(n_updates_per_batch):
                if n_train_multi_size not in [None, "none"]:
                    loss = train_multi_batch(
                        model,
                        optimiser,
                        loss_fn,
                        batch_data,
                        batch_labels,
                        device=device,
                        n_train_multi_size=n_train_multi_size,
                    )
                else:
                    loss = train_batch(
                        model, optimiser, loss_fn, batch_data, batch_labels, device=device
                    )
                losses.append(loss)
            ...
            if batch_number % 10 == 0 and verbose:
                ...
                fig.savefig(
                    os.path.join(
                        save_dir,
                        f"batch_losses_for_{model_type}_{other_bandtype}_F{fmin}_{fmax}.png",
                    )
                )
        ...
        print(
            f"Epoch: {epoch}, Loss: {mloss} val_loss: {mvloss}, epoch_time: {time.time() - epoch_start}"
        )
        ...
        if epoch % save_interval == 0:
            ...
            fig.savefig(
                os.path.join(
                    save_dir, f"losses_for_{model_type}_{other_bandtype}_F{fmin}_{fmax}.png"
                )
            )
            ...
            with open(
                ...
                np.savetxt(
                    f, np.array([all_losses, all_val_losses]).T, header="train_loss, val_loss"
                )
...
def main():
    ...
    parser.add_argument(
        "-bt", "--band-type", help="force to train either even or odd band", default=None
    )
    parser.add_argument(
        "-ct", "--continue-train", help="load_model and continue training", action="store_true"
    )
    ...
    parser.add_argument(
        "-nu", "--n-updates-per-batch", help="number of updates per batch", default=1, type=int
    )
    ...
    if args.continue_train and args.overwrite:
        raise Exception(
            "Cannot continue training and overwrite model, please select only one of continue-train or overwrite"
        )
    ...
    if args.continue_train and args.load_model is not None:
        raise Exception(
            "Must provide one of model path directory (--load-model) or continue training from default checkpoint (--continue-train), not both"
        )
