...
class LoadData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tot_data = [
            torch.cat(
                [
                    torch.Tensor(noise_data[i]).squeeze(),
                    torch.Tensor(signal_data[i]).squeeze(),
                ],
                dim=0,
            )
        ...
        truths = torch.cat(
            [torch.zeros(len(noise_data[0])), torch.ones(len(signal_data[0]))]
        )
        if self.shuffle:
        ...
        truths = torch.nn.functional.one_hot(
            torch.Tensor(truths).to(torch.int32).long(), 2
        )
...
def train_multi_batch(
    ...
    for i in range(n_train_multi_size):
        ...
        for sind in start_inds:
            if model_type in ["spectrogram", "vitmapspectrogram"]:
                output = model(
                    torch.Tensor(batch_data[0][:, :, :, sind : sind + seglen]).to(
                        device
                    )
                )
...
def train_model(
    ...
    train_noise_dir = os.path.join(
        load_dir, "train", bandtype, f"band_{fmin}_{fmax}", "snr_0.0_0.0"
    )
    train_signal_dir = os.path.join(
        ...
        bandtype,
        f"band_{fmin}_{fmax}",
        f"snr_{float(snrmin)}_{float(snrmax)}",
    )

    val_noise_dir = os.path.join(
        load_dir, "train", other_bandtype, f"band_{fmin}_{fmax}", "snr_0.0_0.0"
    )
    ...
    validation_dataset = LoadData(
        val_noise_dir, val_signal_dir, load_types=load_types, nfile_load=2
    )
...
def main():
    ...
    parser.add_argument(
        "-c", "--config-file", help="config file contatining parameters"
    )
