...
def load_model_from_config(config, weights, device="cpu"):
    ...
    model_soap = soapcw.cnn.pytorch.models.CNN(
        ...
        fc_layers=config["model"][
            "fc_layers"
        ],  # the size of the fully connected mlp layers
        conv_layers=config["model"][
            "conv_layers"
        ],  # the convolutional layers (num_filters, filter_size, maxpool_size, stride)
        inchannels=config["model"]["n_channels"],
        avg_pool_size=config["model"][
            "avg_pool_size"
        ],  # the size of the average pooling layer
        device=device,
    ).to(device)
...
def load_vitmap_model(fmin, obs_run="O3", checkpoint_file=None):
    ...
    if checkpoint_file is None:
        model_state = pkg_resources.resource_stream(
            __name__,
            "trained_models/{}/vitmap_F{}_{}.ckpt".format(obs_run, brange, oddeven),
        )
