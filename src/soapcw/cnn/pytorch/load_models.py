...
def load_model_from_config(weights_file, config=None, device="cpu"):
    ...
    model_soap = soapcw.cnn.pytorch.models.CNN(
        ...
        conv_layers=pre_model_weights[
            "conv_layers"
        ],  # the convolutional layers (num_filters, filter_size, maxpool_size, stride)
...
def load_vitmap_model(fmin, obs_run="O3", checkpoint_file=None):
    ...
    if checkpoint_file is None:
        model_state = pkg_resources.resource_stream(
            __name__, "trained_models/{}/vitmap_F{}_{}.ckpt".format(obs_run, brange, oddeven)
        )
    model_init = pkg_resources.resource_stream(
        __name__, "trained_models/{}/vitmap.ini".format(obs_run)
    )
    ...
    model = cnn.CNN(
        (156, 89), 1, config["lin_layers"], config["conv_layers"], device="cpu", num_channels=0
    )
...
def load_spect_model(fmin, obs_run="O3"):
    ...
    model = cnn.CNN(
        (156, 89), 1, config["lin_layers"], config["conv_layers"], device="cpu", num_channels=2
    )
    model.load_state_dict(torch.load(model_state))
...
def read_config(config_file):
    ...
    p["conv_layers"] = [
        (conv_num_filt[i], conv_filt_size[i], conv_max_pool[i], 1)
        for i in range(len(conv_num_filt))
    ]
