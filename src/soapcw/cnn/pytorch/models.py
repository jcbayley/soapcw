...
class CNN(nn.Module):
    ...
    def create_network(self, name, input_dim, append_dim=0, fc_layers=[], conv_layers=[]):
        ...
        if self.avg_pool_size is not None and num_conv > 0:
            ...
            lin_input_size = (
                np.prod(layer_out_sizes[-1]) if num_conv > 0 else np.prod(self.input_dim)
            )
    ...
    def conv_out_size(self, height_in, width_in, padding, dilation, kernel, stride, maxpool_size):
        height_out = int(
            ((height_in + 2 * padding[0] - dilation[0] * (kernel[0] - 1) - 1) / stride[0] + 1)
            / maxpool_size[0]
        )
        width_out = int(
            ((width_in + 2 * padding[1] - dilation[1] * (kernel[1] - 1) - 1) / stride[1] + 1)
            / maxpool_size[1]
        )
...
class VitmapSpectStatCNN(nn.Module):
    ...
    def forward(self, y_vitmap, y_spect, y_stat):
        ...
        y_vitmap = torch.reshape(
            y_vitmap, (-1, self.vitmap_network.inchannels, self.input_dim[0], self.input_dim[1])
        )
        ...
        y_spect = torch.reshape(
            y_spect, (-1, self.spect_network.inchannels, self.input_dim[0], self.input_dim[1])
        )
