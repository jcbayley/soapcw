...
class CNN(nn.Module):
    def __init__(
        ...
        self.conv_network, self.lin_network = self.create_network(
            "o", self.input_dim, fc_layers=self.fc_layers, conv_layers=self.conv_layers
        )

    def create_network(
        self, name, input_dim, append_dim=0, fc_layers=[], conv_layers=[]
    ):
        ...
        for i in range(num_conv):
            ...
            outsize = self.conv_out_size(
                insize[0], insize[1], padding, dilation, filt_size, stride, maxpool_size
        ...
        if self.avg_pool_size is not None and num_conv > 0:
            ...
            lin_input_size = (
                np.prod(layer_out_sizes[-1])
                if num_conv > 0
                else np.prod(self.input_dim)
            )
    ...
    def conv_out_size(
        self, height_in, width_in, padding, dilation, kernel, stride, maxpool_size
    ):
        height_out = int(
            (
                (height_in + 2 * padding[0] - dilation[0] * (kernel[0] - 1) - 1)
                / stride[0]
                + 1
            )
            / maxpool_size[0]
        ...
        width_out = int(
            (
                (width_in + 2 * padding[1] - dilation[1] * (kernel[1] - 1) - 1)
                / stride[1]
                + 1
            )
            / maxpool_size[1]
    ...
    def forward(self, y):
        ...
        lin_in = (
            torch.flatten(conv, start_dim=1) if self.num_conv > 0 else y
...
class VitmapSpectStatCNN(nn.Module):
    def __init__(
        ...
        if self.spect_network is not None:
            self.spect_network = torch.load(
                self.spect_network, map_location=self.device
            )
        else:
            self.spect_network = CNN(
                self.input_dim,
                self.output_dim,
        ...
        if self.vitmap_network is not None:
            self.vitmap_network = torch.load(
                self.vitmap_network, map_location=self.device
            )
        else:
            self.vitmap_network = CNN(
                self.input_dim,
                self.output_dim,
                fc_layers=self.fc_layers,
                conv_layers=self.conv_layers,
        ...
        self.spect_network.lin_network = nn.Sequential(
            *list(self.spect_network.lin_network.children())[:-1]
        )
        self.vitmap_network.lin_network = nn.Sequential(
            *list(self.vitmap_network.lin_network.children())[:-1]
        )

        for net in [
            self.spect_network.lin_network,
            self.spect_network.conv_network,
            self.vitmap_network.lin_network,
            self.vitmap_network.conv_network,
        ]:
        ...
        self.final_classifier.add_module(
            "linout", module=nn.Linear(2 * self.fc_layers[-1] + 1, self.output_dim)
        )
    ...
    def forward(self, y_vitmap, y_spect, y_stat):
        ...
        y_vitmap = torch.reshape(
            y_vitmap,
            (-1, self.vitmap_network.inchannels, self.input_dim[0], self.input_dim[1]),
        )
        vitmap_conv = self.vitmap_network.conv_network(y_vitmap)
        vitmap_lin = torch.flatten(vitmap_conv, start_dim=1)
        vitmap_out = self.vitmap_network.lin_network(
            vitmap_lin
        )  # run fully connected network

        # run the spect network
        y_spect = torch.reshape(
            y_spect,
            (-1, self.spect_network.inchannels, self.input_dim[0], self.input_dim[1]),
        )
        spect_conv = self.spect_network.conv_network(y_spect)
        spect_lin = torch.flatten(spect_conv, start_dim=1)
        spect_out = self.spect_network.lin_network(
            spect_lin
        )  # run fully connected network
    ...
    def test(model, y_vitmap, y_spect, y_stat):
        ...
        for i in range(num_data):
            x_out.append(
                model.forward(y_vitmap[i], y_spect[i], y_stat[i]).cpu().numpy()
            )
        return np.array(x_out)
...
class VitmapStatCNN(nn.Module):
    def __init__(
        ...
        # we then add this to the output of stat network
        self.final_classifier.add_module(
            "linout", module=nn.Linear(self.fc_layers[-1] + 1, self.output_dim)
        )
        self.final_classifier.add_module("act_linout", module=self.out_activation)
