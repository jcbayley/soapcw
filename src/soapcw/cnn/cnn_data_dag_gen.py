...
def write_subfile(
    sub_filename, config_file, config, datatype, comment, dirs, verbose=False
):
    ...
    with open(sub_filename, "w") as f:
        ...
        execute = os.path.join(
            os.path.split(sys.executable)[0], config["code"]["search_exec"]
        )
        f.write("executable = {}\n".format(execute))
        # f.write('enviroment = ""\n')
        f.write("getenv  = True\n")
...
def write_dagfile(config, datatype="train"):
    ...
    for i, bs in enumerate(config["data"]["band_starts"]):
        bandstart = np.arange(
            config["data"]["band_starts"][i],
            config["data"]["band_ends"][i],
            config["condor"]["data_load_size"] - 0.1,
        )[:-1]
