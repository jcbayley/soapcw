...
def model_sig():
    ...
    inputim = Input(
        shape=(psf.image_shape[0], psf.image_shape[1], 1), name="image_input"
    )
...
def plot_hists(zero_data, snr_data, snr):
    ...
    ax.hist(
        snr_sata, histtype="stepfilled", alpha=0.6, label="SNR {}".format(snr), bins=20
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--train-dir", help="training data directory", type=str, required=False
    )
    parser.add_argument(
        "-v", "--val-dir", help="validations data directory", type=str, required=False
    )
    parser.add_argument(
        "-e", "--test-dir", help="test data directory", type=str, required=False
    )
    parser.add_argument(
        "-o", "--output-dir", help="test data directory", type=str, required=False
    )
    parser.add_argument(
        "-T", "--type", help="set training or testing", type=str, required=True
    )
    parser.add_argument("-s", "--snr", help="signal snrs", type=float, required=True)
    parser.add_argument(
        "-N",
        "--norm-sft",
        help="normalise data to maximum",
        type=str2bool,
        required=False,
        default=False,
    )
    parser.add_argument(
        "-vitmap",
        "--vitmap-dir",
        help="directory to vitmap model",
        type=str,
        required=False,
    )
    parser.add_argument(
        "-stat", "--stat-dir", help="directory to stat model", type=str, required=False
    )
    parser.add_argument(
        "-tr",
        "--test-run",
        help="which data run the test was on",
        type=str,
        required=False,
        default="s6mdc",
    )
    ...
    if args.type == "efficiency":
        ...
        train_cnn(
            ...
            args.output_dir,
            args.snr,
            norm_sft=args.norm_sft,
            vitmap_dir=args.vitmap_dir,
