...
def run_model(
    ...
    test_save_dir = os.path.join(
        save_dir, f"test_model_{model_type}_for_{other_bandtype}_F{fmin}_{fmax}"
    )
    ...
    test_data = soapcw.cnn.train_model.LoadData(
        train_noise_dir, train_signal_dir, load_types=load_types, shuffle=True, nfile_load=n_test
    )
    ...
    with torch.no_grad():
        ...
        for i, (batch_data, batch_labels, batch_pars) in enumerate(test_dataset):
            ...
            vloss, stat = run_batch(
                model, optimiser, loss_fn, batch_data[0], batch_labels, train=False, device=device
            )
        ...
        with h5py.File(
            os.path.join(
                test_save_dir, f"test_results_{model_type}_{other_bandtype}_F{fmin}_{fmax}.h5"
            ),
            "w",
        ) as f:
            ...
            f.create_dataset(
                "lineaware_sensitivity", data=np.column_stack((line_snr_range, line_sensitivity))
            )
            f.create_dataset(
                "cnn_sensitivity", data=np.column_stack((cnn_snr_range, cnn_sensitivity))
            )
    ...
    fig.savefig(
        os.path.join(
            test_save_dir, f"test_snr_vs_cnnstat_{model_type}_{other_bandtype}_F{fmin}_{fmax}.png"
        )
    )
    ...
    ax.hist(
        test_statistic[:, 1][test_labels[:, 1] == 0],
        bins=100,
        histtype="step",
        label="Noise",
        range=(cnn_bin_min, cnn_bin_max),
    )
    ax.hist(
        test_statistic[:, 1][test_labels[:, 1] == 1],
        bins=100,
        histtype="step",
        label="Signal",
        range=(cnn_bin_min, cnn_bin_max),
    ...
    fig.savefig(
        os.path.join(
            test_save_dir,
            f"test_cnnstat_histogram_{model_type}_{other_bandtype}_F{fmin}_{fmax}.png",
        )
    )
    ...
    ax.hist(
        test_lineaware[test_labels[:, 1] == 0],
        bins=100,
        histtype="step",
        label="Noise",
        range=(line_aware_bin_min, line_aware_bin_max),
    )
    ax.hist(
        test_lineaware[test_labels[:, 1] == 1],
        bins=100,
        histtype="step",
        label="Signal",
        range=(line_aware_bin_min, line_aware_bin_max),
    ...
    fig.savefig(
        os.path.join(
            test_save_dir,
            f"test_lineawarestat_histogram_{model_type}_{other_bandtype}_F{fmin}_{fmax}.png",
        )
    )
    ...
    fig.savefig(
        os.path.join(
            test_save_dir,
            f"test_lineawarestat_vs_cnnstat_{model_type}_{other_bandtype}_F{fmin}_{fmax}.png",
        )
    )
    ...
    fig.savefig(
        os.path.join(
            test_save_dir,
            f"test_snr_vs_lineawarestat_{model_type}_{other_bandtype}_F{fmin}_{fmax}.png",
        )
    )
    ...
    ax.plot(
        test_pars["snr"],
        (np.array(test_lineaware).flatten() + 650) * np.array(test_statistic)[:, 1],
        ".",
    )
    ax.set_xlabel("SNR")
    ...
    fig.savefig(
        os.path.join(
            test_save_dir,
            f"test_snr_vs_bothmultiply_{model_type}_{other_bandtype}_F{fmin}_{fmax}.png",
        )
    )
    ...
    fig.savefig(
        os.path.join(
            test_save_dir, f"test_snr_vs_odds_{model_type}_{other_bandtype}_F{fmin}_{fmax}.png"
        )
    )
    ...
    fig.savefig(
        os.path.join(
            test_save_dir, f"test_sensitivity_{model_type}_{other_bandtype}_F{fmin}_{fmax}.png"
        )
    )
...
def make_sensitivity_plot(statistic, labels, snr, false_alarm=0.01, window_width=5):
    ...
    for snr_value in snr_range:
        ...
        if len(window_statistics) > 0:
            fraction_above_threshold = np.mean(
                window_statistics[window_labels == 1] > false_alarm_threshold
            )
        else:
