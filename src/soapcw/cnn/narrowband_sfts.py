...
def get_filelist(out_path, sftdir, data_type):
    ...
    if os.path.isfile(filelist_fname):
        raise Exception(
            "File already exists: Please delete this file if you want to rewrite"
        )
...
def split_sfts_list(config, detector, filelist_fname, output_dir):
    ...
    for fname in sft_list:
        try:
            runstr = "lalapps_splitSFTs -d {} -fs {} -fe {} -fb {} -fx {} -n {} -as 0 -- {}".format(
                detector,
                config["params"]["band_start"],
                config["params"]["band_end"],
                config["params"]["band_width"],
                config["params"]["band_overlap"],
                output_dir,
                fname,
            )
...
def write_subfile(config_file, params, output_dir, detector, filelist_fname):
    ...
    comment = "narrowband_{}_{}_{}_{}.sub".format(
        detector,
        params["params"]["obsrun"],
        params["params"]["band_start"],
        params["params"]["band_end"],
    )
    ...
    with open(sub_filename, "w") as f:
        ...
        f.write("getenv  = True\n")
        f.write(
            "log = {}/condor/log/{}_$(cluster).log\n".format(
                params["dirs"]["output_dir"], comment
            )
        )
        f.write(
            "error = {}/condor/err/{}_$(cluster).err\n".format(
                params["dirs"]["output_dir"], comment
            )
        )
        f.write(
            "output = {}/condor/out/{}_$(cluster).out\n".format(
                params["dirs"]["output_dir"], comment
            )
        )
        f.write("request_disk = 1000 \n")
        f.write(
            f"arguments = --config-file {config_file} --sft-filelist {filelist_fname} --output-dir {output_dir}\n"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f", "--config-file", help="path to config file", type=str, required=True
    )
    parser.add_argument(
        "-dag",
        "--make-dag",
        help="make the dag file for each detector",
        action="store_true",
    )
    parser.add_argument(
        "-d", "--detector", help="which detector to run on", type=str, required=False
    )
    parser.add_argument(
        "-s", "--sft-filelist", help="filelist of sfts", type=str, required=False
    )
    parser.add_argument(
        "-o",
        "--output-dir",
        help="output_directory for narrowbanded sfts",
        type=str,
        required=False,
    )
