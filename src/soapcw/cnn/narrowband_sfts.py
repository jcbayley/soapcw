    ...
    for fname in sft_list:
        try:
            runstr = "lalpulsar_splitSFTs -fs {} -fe {} -fb {} -fx {} -n {} -as 0 -- {}".format(
                fband_start, fband_end, fband_width, fband_overlap, output_dir, fname
            )
...
def write_subfile(config_file, params, output_dir, detector, filelist_fname, band_min, band_max):
    ...
    comment = "narrowband_{}_{}_{}_{}.sub".format(
        detector, params["data"]["obs_run"], band_min, band_max
    )
...
def main():
    ...
    parser.add_argument(
        "-dag", "--make-dag", help="make the dag file for each detector", action="store_true"
    )
    parser.add_argument(
        "-mf", "--make-filelist", help="make the dag file for each detector", action="store_true"
    )
    parser.add_argument(
    ...
    if args.make_dag == True:
        ...
        for i, detector in enumerate(cp["data"]["detectors"]):
            ...
            filelist_fname = os.path.join(
                cp["narrowband"]["narrowband_sft_dir"],
                f"{cp['data']['obs_run']}_{detector}_sft_list.txt",
            )
            ...
            write_subfile(
                ...
                detector,
                filelist_fname,
                cp["narrowband"]["band_min"],
                cp["narrowband"]["band_max"],
            )
        ...
        split_sfts_list(
            ...
            cp["narrowband"]["band_min"],
            cp["narrowband"]["band_max"],
            cp["narrowband"]["band_width"],
            cp["narrowband"]["band_overlap"],
        )
