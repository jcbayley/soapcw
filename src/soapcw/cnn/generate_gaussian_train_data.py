...
def loop_band_train_augment(
    ...
    range_index = np.arange(
        0, (bandmax - width - bandmin) * int(tsft), 180 * stride
    ).astype(int)
    ...
    if config["general"]["load_dir"] is not None:
        ...
        noise_est = sfts_noise.H1.get_sh(pos="median"), sfts_noise.L1.get_sh(
            pos="median"
        )
    ...
    for i in range(len(range_bands)):
        ...
        if loadpath is not None:
            av_sh = [
                np.nanmedian(sfts_noise.H1.rng_med[:, ms:me], axis=1),
                np.nanmedian(sfts_noise.L1.rng_med[:, ms:me], axis=1),
            ]
        else:
            av_sh = [1, 1]
        ...
        for i in range(nperband):
            noise_outputs, signal_outputs = run_and_inj(
                flow,
                width,
                outpath,
                resize_image=config["data"]["resize_image"],
                av_sh=av_sh,
                snrrange=snrrange,
                test=test,
    ...
    even_noise_filenames = os.path.join(
        *[
            ...
            brange,
            noise_out_snr,
            f"freq_{bandmin}_{bandmax}_{nperband*len(range_bands)}.hdf5",
        ]
    )
...
def run_and_inj(
    ...
    if np.shape(data.L1.downsamp_summed_norm_sft_power) != np.shape(
        data.H1.downsamp_summed_norm_sft_power
    ):
        print(
            fmin,
            np.shape(data.L1.downsamp_summed_norm_sft_power),
            np.shape(data.H1.downsamp_summed_norm_sft_power),
        )
        sys.exit()
    ...
    print(
        ...
        fmin,
        fmax,
        data.H1.frequencies[0],
        data.H1.frequencies[-1] + 1.0 / data.tsft,
    )
    epochs = data.H1.summed_epochs
    ...
    signal_outputs = cnn_data_gen.return_outputs(
        out_snr=out_snr_inj,
        datah=data.H1.downsamp_summed_norm_sft_power,
        datal=data.L1.downsamp_summed_norm_sft_power,
        fmin=fmin,
