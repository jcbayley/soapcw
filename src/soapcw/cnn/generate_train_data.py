...
def loop_band_train_augment(
    ...
    hname, lname = cnn_data_gen.find_sft_file(
        config["narrowband"]["narrowband_sft_dir"], bandmin, bandmax
    )
    ...
    even_noise_filenames = os.path.join(
        *[
            path,
            "even",
            brange,
            noise_out_snr,
            f"freq_{bandmin:.1f}_{bandmax:.1f}_{len(config['cnn_data']['shift_bins'])*len(range_bands)}.hdf5",
        ]
    )
    even_signal_filenames = os.path.join(
        *[
            path,
            "even",
            brange,
    ...
    odd_noise_filenames = os.path.join(
        *[
            path,
            "odd",
            brange,
    ...
    odd_signal_filenames = os.path.join(
        *[
            path,
            "odd",
            brange,
    ...
    if (
        os.path.isfile(even_noise_filenames)
        and os.path.isfile(even_signal_filenames)
        and os.path.isfile(odd_noise_filenames)
        and os.path.isfile(odd_signal_filenames)
    ):
    ...
    for i in range(len(range_bands)):
        ...
        for shft in shifts:
            ...
            av_sh = [
                (2 / tsft) * np.nanmedian(sfts.H1.rng_med[:, cts:cte], axis=1),
                (2 / tsft) * np.nanmedian(sfts.L1.rng_med[:, cts:cte], axis=1),
            ]
            ...
            rng_med = {
                "H1": copy.deepcopy(sfts.H1.rng_med[:, cts:cte]),
            ...
            datah, shH, rngH = roll_in_time(
                copy.deepcopy(sfts.H1.sft[:, cts:cte]),
                numbins=100,
                sh=av_sh[0],
                rng_med=copy.deepcopy(sfts.H1.rng_med[:, cts:cte]),
            )
            datal, shL, rngL = roll_in_time(
                copy.deepcopy(sfts.L1.sft[:, cts:cte]),
                numbins=100,
                sh=av_sh[1],
                rng_med=copy.deepcopy(sfts.L1.rng_med[:, cts:cte]),
            ...
            for key in save_options:
                if split == "even":
                    even_noise_save_outs[key].extend(
                        [noise_outs1[key], noise_outs2[key], noise_outs3[key], noise_outs4[key]]
                    )
                    even_signal_save_outs[key].extend(
                        [signal_outs1[key], signal_outs2[key], signal_outs3[key], signal_outs4[key]]
                    )
                elif split == "odd":
                    odd_noise_save_outs[key].extend(
                        [noise_outs1[key], noise_outs2[key], noise_outs3[key], noise_outs4[key]]
                    )
                    odd_signal_save_outs[key].extend(
                        [signal_outs1[key], signal_outs2[key], signal_outs3[key], signal_outs4[key]]
                    )
                else:
    ...
    for fname, temp_data in zip(
        [even_noise_filenames, even_signal_filenames, odd_noise_filenames, odd_signal_filenames],
        [even_noise_save_outs, even_signal_save_outs, odd_noise_save_outs, odd_signal_save_outs],
    ):
        ...
        with h5py.File(fname, "w") as f:
            for key in save_options:
                if key == "pars":
                    ...
                    f.create_dataset(
                        "pars",
                        data=np.array([[td[key] for key in parkeys] for td in temp_data[key]]),
                    )
                else:
                    try:
                        f.create_dataset(
                            key, shape=np.shape(temp_data[key]), data=np.array(temp_data[key])
                        )
...
def run_and_inj(
    ...
    nsft, tstart, tsft, flow, fhigh = sfts.H1.nsft, sfts.H1.epochs[0], 1800.0, fmin, fmin + width
    fmax = fmin + width
    ...
    for key in param_list:
        if key == "f":
            ...
            pars[key] = np.random.uniform(
                config["cnn_data"][f"{key}_range"][0], config["cnn_data"][f"{key}_range"][1]
            )
