...
def loop_band_train_augment(
    ...
    range_index = np.arange(
        0, (bandmax - width - bandmin) * int(tsft), nbin * stride
    ).astype(int)
    ...
    hname, lname = cnn_data_gen.find_sft_file(
        config["general"]["narrowband_sft_dir"], bandmin, bandmax
    )
    ...
    for i in range(len(range_bands)):
        ...
        for cts in shifts:
            ...
            datah, shH = flip_data_time(
                copy.deepcopy(sfts.H1.norm_sft_power[:, cts:cte]), sh=av_sh[0]
            )
            datal, shL = flip_data_time(
                copy.deepcopy(sfts.L1.norm_sft_power[:, cts:cte]), sh=av_sh[1]
            )
            ...
            datah, shH = roll_in_time(
                copy.deepcopy(sfts.H1.norm_sft_power[:, cts:cte]),
                numbins=100,
                sh=av_sh[0],
            )
            datal, shL = roll_in_time(
                copy.deepcopy(sfts.L1.norm_sft_power[:, cts:cte]),
                numbins=100,
                sh=av_sh[1],
            )
    ...
    even_noise_filenames = os.path.join(
        *[
            ...
            brange,
            noise_out_snr,
            f"freq_{bandmin}_{bandmax}_{len(shifts)*len(range_bands)}.hdf5",
        ]
    )
            ...
            for key in save_options:
                if key == "pars":
                    ...
                    f.create_dataset(
                        "pars",
                        data=np.array(
                            [[td[key] for key in parkeys] for td in temp_data[key]]
                        ),
                    )
                else:
                    try:
                        f.create_dataset(
                            key,
                            shape=np.shape(temp_data[key]),
                            data=np.array(temp_data[key]),
                        )
                    except ValueError as e:
                        print(key)
                        print(len(temp_data[key]))
                        lengths = [
                            temp_data[key][i] for i in range(len(temp_data[key]))
                        ]
...
def run_and_inj(
    ...
    for j in np.linspace(0, len(param_list) - 1, len(param_list)).astype(int):
        pars[param_list[j]] = params_low[j] + np.random.uniform(0, 1) * (
    ...
    sig.earth_ephem = "/home/joseph.bayley/repositories/lalsuite/lalpulsar/lib/earth00-40-DE430.dat.gz"
    sig.sun_ephem = (
        "/home/joseph.bayley/repositories/lalsuite/lalpulsar/lib/sun00-40-DE430.dat.gz"
    )
    sig.snr = snr
    sig.fmin = fmin
    sig.fmax = fmax
