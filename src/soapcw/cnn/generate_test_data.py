...
def loop_band_test_data(
    loadpath, path, bandmin, bandmax, band_width, resize_image=True, search=False, num_repeat=1
):
    if 40 <= bandmin <= 500:
        stride = 1
        degfree = 96
    ...
    sfts = cw.LoadSFT(
        "{};{}".format(hname, lname), norm=True, filled=True, vetolist=vetolist, save_rngmed=True
    )

    bin_width = 1.0 / float(1800.0)
    ...
    for i in range(len(range_bands)):
        ...
        av_sh = (2 / 1800.0) * np.nanmedian([sfts.H1.rng_med[:, ms:me], sfts.L1.rng_med[:, ms:me]])

        datah = copy.deepcopy(sfts.H1.norm_sft_power[:, ms:me])
        datal = copy.deepcopy(sfts.L1.norm_sft_power[:, ms:me])
        print(
            "SFTLEN. H1:{}, L1:{}".format(sum(datah[:, 0] != degfree), sum(datal[:, 0] != degfree))
        )
    ...
    for key in save_options:
        even_noise_filenames[key] = os.path.join(
            *[
                path,
                "even",
                key,
                brange,
                noise_out_snr,
                "freq_{}_{}.pkl".format(bandmin, len(even_noise_save_outs[key])),
            ]
        )
        even_signal_filenames[key] = os.path.join(
            *[
                path,
                "even",
                key,
                brange,
                signal_out_snr,
                "freq_{}_{}.pkl".format(bandmin, len(even_signal_save_outs[key])),
            ]
        )
        odd_noise_filenames[key] = os.path.join(
            *[
                path,
                "odd",
                key,
                brange,
                noise_out_snr,
                "freq_{}_{}.pkl".format(bandmin, len(odd_noise_save_outs[key])),
            ]
        )
        odd_signal_filenames[key] = os.path.join(
            *[
                path,
                "odd",
                key,
                brange,
                signal_out_snr,
                "freq_{}_{}.pkl".format(bandmin, len(odd_signal_save_outs[key])),
            ]
        )

        for fname in [
            even_noise_filenames[key],
            even_signal_filenames[key],
            odd_noise_filenames[key],
            odd_signal_filenames[key],
        ]:
...
def run_and_inj_test(
    ...
    if snr == 0:
        ...
        sig.earth_ephem = (
            "/home/joseph.bayley/repositories/lalsuite/lalpulsar/lib/earth00-40-DE430.dat.gz"
        )
        sig.sun_ephem = (
            "/home/joseph.bayley/repositories/lalsuite/lalpulsar/lib/sun00-40-DE430.dat.gz"
        )
        data = sig.get_spectrogram(
            epochs=sfts.H1.epochs,
            nsft=len(datah),
            tref=sfts.H1.epochs[0],
            tsft=tsft,
            fmin=flow,
            fmax=fhigh,
            snr=0,
        ...
        H1path, L1path = (flow + width / 2.0, flow + width / 2.0), (
            flow + width / 2.0,
            flow + width / 2.0,
        )
        save_pars = {
        ...
        sig_outputs = cnn_data_gen.return_outputs(
            out_snr="./",
            datah=data.H1.downsamp_summed_norm_sft_power,
            datal=data.L1.downsamp_summed_norm_sft_power,
            fmin=fmin,
