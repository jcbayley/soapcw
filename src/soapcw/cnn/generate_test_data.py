...
def loop_band_test_data(
    ...
    range_index = np.round(np.arange(0, nbins, width / (2 * bin_width))).astype(int)[
        :-1
    ]
    ...
    for i in range(len(range_bands)):
        ...
        av_sh = (2 / 1800.0) * np.nanmedian(
            [sfts.H1.rng_med[:, ms:me], sfts.L1.rng_med[:, ms:me]]
        )
        ...
        print(
            "SFTLEN. H1:{}, L1:{}".format(
                sum(datah[:, 0] != degfree), sum(datal[:, 0] != degfree)
            )
        )
