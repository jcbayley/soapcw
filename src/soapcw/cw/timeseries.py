...
class TimeSeries:
    ...
    def sfts_from_timeseries(
            ...
            temp_seg = np.array(
                self.timeseries[
                    int(i * segment_length_index * (1 - overlap)) : int(
                        int(i * segment_length_index * (1 - overlap))
                        + segment_length_index
                    )
                ]
            )
        ...
        for t, seg in enumerate(time_segments):
            ...
            if real:
                ...
                fft_freqs[t] = np.abs(
                    np.fft.fftfreq(len(fft_calc), self.delta_t)[
                        : int(len(fft_calc) / 2 + 1)
                    ]
                )
    ...
    def write_sft_from_timeseries(
        ...
        sft = self.sfts_from_timeseries(
            tsft, window=window, fmin=fmin, fmax=fmax, real=False
        )
