...
class TimeSeries:
    ...
    def LAL_sfts_from_timeseries(self):
        ...
        real8time = lal.CreateREAL8TimeSeries(
            "", self.start_time, 0, self.delta_t, lal.SecondUnit, self.length
        )
        real8time.data = vect
    ...
    def sfts_from_timeseries(
        self, tsft, window=None, fmin=None, fmax=None, real=False, tstart=None, overlap=0
    ):
        """
        ...
        if self.sample_frequency is None:
            self.sample_frequency = 1.0 / self.delta_t

        if self.delta_t is None:
            self.delta_t = 1.0 / self.sample_frequency
        ...
        time_segments = []
        segment_number = np.floor(
            float(len(self.timeseries)) / (segment_length_index * (1 - overlap))
        ).astype(int)
        for i in range(segment_number):
            temp_seg = np.array(
                self.timeseries[
                    int(i * segment_length_index * (1 - overlap)) : int(
                        int(i * segment_length_index * (1 - overlap)) + segment_length_index
                    )
                ]
            )
        ...
        for t, seg in enumerate(time_segments):
            ...
            if real:
                ...
                fft_freqs[t] = np.abs(
                    np.fft.fftfreq(len(fft_calc), self.delta_t)[: int(len(fft_calc) / 2 + 1)]
                )
    ...
    def write_sft_from_timeseries(
        self, output_path, tsft, window=None, fmin=None, fmax=None, narrowband=False
    ):
        """
...
class LoadTimeSeries(TimeSeries):
    def __init__(self, filename, channel_name, start_time=None, end_time=None):
        ...
        """
        self.read_frame(
            filename=filename, channel_name=channel_name, start_time=start_time, end_time=end_time
        )

    def read_frame(self, filename, channel_name, start_time=None, end_time=None):
        fr = read_frame_pycbc(
            filename, channels=channel_name, start_time=start_time, end_time=end_time
        )
