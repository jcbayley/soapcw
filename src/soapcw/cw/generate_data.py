...
from .tools import LAL_EPHEMERIS_URL, download_ephemeris_file
...
class GenerateSignal:
    ...
    def av_antenna(
        self, alpha, delta, det, tstart, nsft, tsft, antenna=True, use_lal=False, average=True
    ):
        """
        ...
        if not antenna:
            ...
            am = [
                lal.ComputeDetAMResponse(self.siteinfo[det].response, alpha, delta, 0.0, i)
                for i in gmst_rad
            ]
    ...
    def get_timeseries(
        self, duration=None, tstart=None, tref=None, Sn=None, detectors=None, sample_frequency=4096
    ):
        """
        ...
        if self.snr == 0 or self.h0 == 0:
            ...
            for i, ind_floor in zip(list(range(len(sft.pulsar_index))), sft.pulsar_index):
                ...
                for lim in limits:
                    ...
                    update_index.append(
                        [int(i), int(bin_floor_index), bin_cent_freq, pul_freq, snrsq, noise_offset]
                    )
            ...
            if len(update_index) != 0:
                ...
                ncps = (
                    self.fresnel_power(update_index[:, 2], update_index[:, 3], self.tsft, self.f[1])
                    * update_index[:, 4]
                    + update_index[:, 5]
                )
                ...
                if rand:
                    sft.norm_sft_power[tuple(update_index[:, :2].astype(int).T)] = st.ncx2.rvs(
                        int(2), nc=update_index[:, 6], loc=0, scale=1
                    )
                elif not rand:
                    sft.norm_sft_power[tuple(update_index[:, :2].astype(int).T)] = update_index[
                        :, 6
                    ]
...
class SimulateSpectrogramfromSFT(SimulateGaussianNoiseSpectrogram):
    ...
    def get_h0_from_SNR(self, snr, sftpaths):
        ...
        snr_ = pyfstat.SignalToNoiseRatio.from_sfts(
            F0=f0, sftfilepath=sftpaths, sft_constraint=constraints
        )

        h0 = snr_.compute_h0_from_snr2(
            self.alpha, self.delta, self.psi, self.phi0, self.cosi, snr**2
        )
    ...
    def get_from_cwsimulator(self, sft):
        ...
        for dat_i, i, N in S.get_sfts(
            fmin=self.fmin, fmax=self.fmax, Tsft=self.tsft, window="tukey", window_param=0.001
        ):
    ...
    def get_from_pyfstat(self, sftpath):
        ...
        frequency, timestamps, amplitudes = pyfstat.utils.get_sft_as_arrays(
            writer.sftfilepath,
            fMin=np.round(self.fmin * self.tsft) / self.tsft,
            fMax=np.round(self.fmax * self.tsft) / self.tsft,
        )
