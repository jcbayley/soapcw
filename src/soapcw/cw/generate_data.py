...
class GenerateSignal:
    ...
    def param_check(self):
        for i, v in enumerate(params[:-2]):
            if np.isnan(v):
                raise Exception(
                    "parameter {} is not a valid value".format(params_names[i])
                )
    ...
    def get_baryinput(self, detector):
        ...
        if detector == "SSB":
            ...
            baryinput.site.location[0] = 0.0 / lal.C_SI
            baryinput.site.location[1] = 0.0 / lal.C_SI
            baryinput.site.location[2] = 0.0 / lal.C_SI
        else:
    ...
    def get_pulsar_path(self, epochs, det, edat=None):
        ...
        vDotn_c = (
            cosd * cosa * self.det_vels[:, 0]
            + cosd * sina * self.det_vels[:, 1]
            + sind * self.det_vels[:, 2]
        )
    ...
    def av_antenna(
        ...
        det_params = {
            "H1": {
                "lamb": 46.45 * np.pi / 180.0,
                "L": 119.41 * np.pi / 180.0,
                "gamma": 171.8 * np.pi / 180.0,
                "zeta": 90.0 * np.pi / 180.0,
            },
            "L1": {
                "lamb": 30.56 * np.pi / 180.0,
                "L": 90.77 * np.pi / 180.0,
                "gamma": 243.0 * np.pi / 180.0,
                "zeta": 90 * np.pi / 180.0,
            },
            "V1": {
                "lamb": 43.63 * np.pi / 180.0,
                "L": -10.5 * np.pi / 180.0,
                "gamma": 116.5 * np.pi / 180.0,
                "zeta": 90 * np.pi / 180.0,
            },
        }
        ...
        if not antenna:
            ...
            am = [
                lal.ComputeDetAMResponse(
                    self.siteinfo[det].response, alpha, delta, 0.0, i
                )
                for i in gmst_rad
            ]
            fraczeta = 1.0 / np.sin(det_params[det]["zeta"])
            ...
            if average:
                return np.sum(ABC, axis=0) * 1.0 / len(epochs)
            else:
                return np.array(ABC)
    ...
    def get_snr2(
        ...
        ap = (1.0 / 2) * (1 + params["cosi"] * params["cosi"])
        ac = params["cosi"]
        ...
        if epochs is None:
            if tstart is None or tsft is None or nsft is None:
                ...
                epochs = np.linspace(
                    params["tstart"],
                    params["tstart"] + params["nsft"] * params["tsft"],
                    params["nsft"],
                )
        ...
        snr_2 = np.nan_to_num(
            ((params["h0"] * params["h0"] * 1 * params["tsft"]) / (2.0 * Sn))
            * (alp1 * A + alp2 * B + 2 * alp3 * C)
        )
        return snr_2
    ...
    def get_edat(self):
        ...
        try:
            ...
            self.edat = lalpulsar.InitBarycenter(
                earthEphemerisFile=self.earth_ephem, sunEphemerisFile=self.sun_ephem
            )
        except Exception as e:
            print(
                "Could not load ephemeris file: {} {}, {}".format(
                    self.earth_ephem, self.sun_ephem, e
                )
            )
            try:
                self.earth_ephem = download_ephemeris_file(
                    LAL_EPHEMERIS_URL.format(self.earth_ephem)
                )
                self.sun_ephem = download_ephemeris_file(
                    LAL_EPHEMERIS_URL.format(self.sun_ephem)
                )
                self.edat_p = [self.sun_ephem, self.earth_ephem]
                self.edat = lalpulsar.InitBarycenter(
                    earthEphemerisFile=self.earth_ephem, sunEphemerisFile=self.sun_ephem
    ...
    def gen_timeseries(
        ...
        for det in detectors:
            ...
            S = simulateCW.CWSimulator(
                ...
                self.delta,
                det,
                earth_ephem_file=self.earth_ephem,
                sun_ephem_file=self.sun_ephem,
            )

            if Sn is None:
                ltime, signal = S.get_strain(sample_frequency)
            else:
                ltime, signal = S.get_strain(
                    sample_frequency, noise_sqrt_Sh=np.sqrt(Sn[det])
                )

            timeseries.timeseries = signal
            timeseries.epochs = ltime
            timeseries.sample_frequency = sample_frequency
            timeseries.delta_t = 1.0 / timeseries.sample_frequency
        return timeseries
...
class SimulateSpectrogram:
    ...
    def __gen_spect__(
        ...
        if dets is None and Sn is None and noise_spect is None:
            raise Exception(
                "Please define the detectors (dets), noisefloors (Sn) or real data (noise_spect) to simulate"
            )
        ...
        if epochs is None:
            ...
            self.T = nsft * tsft
            self.tstart = tstart
            self.epochs = np.linspace(
                tstart, tstart + (self.nsft - 1) * tsft, self.nsft
            )
        else:
            self.epochs = epochs
                    ...
                    for key in self.Sn.keys():
                        if len(np.shape(self.Sn[key])) == 0:
                            ...
                            if len(self.Sn[key]) != len(self.epochs):
                                raise Exception(
                                    "Epochs and noise floor not the same length"
                                )
            else:
    ...
    def __sim_data__(
        ...
        self.fmin = np.round(self.fmin * self.tsft).astype(int) / float(
            self.tsft
        )  # min frequency
        self.fmax = np.round(self.fmax * self.tsft).astype(int) / float(
            self.tsft
        )  # max frequency
        # print("round minmaxf", self.fmin, self.fmax)

        self.nbins = np.round((self.fmax - self.fmin) * self.tsft).astype(
            int
        ...
        self.delta_f = 1.0 / self.tsft  # separation of bins in frequency
        self.frequencies = np.arange(
            self.fmin, self.fmax, sft.delta_f
        )  # [:-1] # bin centers for each bin
        ...
        if rand:
            sft.norm_sft_power = st.chi2.rvs(
                int(2), loc=0, scale=1, size=(sft.nsft, sft.nbins)
            )
        if not rand:
        ...
        sft.pulsar_index = np.floor(
            (sft.pulsar_path - self.fmin) * self.tsft
        )  # + 0.5/self.tsft
        ...
        if self.snr == 0 or self.h0 == 0:
            ...
            update_index = []
            for i, ind_floor in zip(
                list(range(len(sft.pulsar_index))), sft.pulsar_index
            ):
                ...
                for lim in limits:
                    ...
                    bin_floor_index = int(
                        ind_floor + lim
                    )  # np.floor(((j1 + lim/tsft) - fmin)/tsft).astype(int)

                    if bin_floor_index < 0 or bin_floor_index >= len(
                        sft.norm_sft_power[0]
                    ):
                        continue
            ...
            if len(update_index) != 0:
                ...
                ncps = (
                    self.fresnel_power(
                        update_index[:, 2], update_index[:, 3], self.tsft, self.f[1]
                    )
                    * update_index[:, 4]
                    + update_index[:, 5]
                )
                ...
                if rand:
                    sft.norm_sft_power[
                        tuple(update_index[:, :2].astype(int).T)
                    ] = st.ncx2.rvs(int(2), nc=update_index[:, 6], loc=0, scale=1)
                elif not rand:
                    sft.norm_sft_power[
                        tuple(update_index[:, :2].astype(int).T)
                    ] = update_index[:, 6]
    ...
    def sum_sfts(self, sum_type="norm_sft_power", gap_val=2, nsfts=48):
        ...
        for dt in self.det_names:
            ...
            data.sum_sft(
                sum_type=sum_type, gap_val=gap_val, nsfts=nsfts, remove_original=False
            )

    def downsamp_frequency(
        self, data_type="summed_norm_sft_power", stride=2, remove_original=False
    ):
        """
