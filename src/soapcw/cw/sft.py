...
class SFT:
    ...
    def sum_sft(
        self, sum_type="norm_sft_power", gap_val=2, nsfts=48, remove_original=False
    ):
        """
        ...
        for i in np.arange(0, self.nsft, nsfts).astype(int):
            ...
            if end >= self.nsft:
                data = np.vstack(
                    [data, gap_val * np.ones((end - self.nsft, len(data[0])))]
                )
    ...
    def downsamp_frequency(
        self, data_type="norm_sft_power", stride=2, remove_original=False
    ):
        """
    ...
    def write_sft_files(self, output_path, narrowband=False, fmin=None, fmax=None):
        ...
        for t, fft in enm:
            if not narrowband:
                ...
                vct.data[t].data.data[:] = np.array(fft[fmin_index:fmax_index]).astype(
                    "complex64"
                )
