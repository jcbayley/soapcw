...
class SFT:
    def __init__(
        self, nsft=None, tsft=None, delta_f=None, fmin=None, fmax=None, det_name=None, tstart=None
    ):
        """
    ...
    def sum_sft(self, sum_type="norm_sft_power", gap_val=2, nsfts=48, remove_original=False):
        ...
        for i in np.arange(0, self.nsft, nsfts).astype(int):
            ...
            if np.isnan(gap_val):
                fr = 1.0 - len(chunk[:, 0][np.isnan(chunk[:, 0])]) / float(nsfts)
            else:
                fr = 1.0 - len(chunk[:, 0][chunk[:, 0] == gap_val]) / float(nsfts)
        ...
        if not hasattr(self, "rng_med"):
            if use_lal:
                ...
                sig2 = lal.CreateREAL8FrequencySeries(
                    None, 0, 0, 1.0 / self.tsft, lal.SecondUnit, self.nbins
                )
    ...
    def write_sft_files(self, output_path, narrowband=False, fmin=None, fmax=None):
        ...
        if self.epochs is None:
            self.epochs = np.linspace(
                self.start_time, self.start_time + self.nsft * self.tsft, self.nsft
            )
        ...
        for t, fft in enm:
            if not narrowband:
                ...
                filename_prefix = "{}/{}_{}SFT_T{}-{}_F{}_{}.sft".format(
                    output_path,
                    self.det_name,
                    self.tsft,
                    int(self.epochs[t]),
                    int(self.epochs[-1] - self.epochs[0]),
                    fmin,
                    fmax,
                )
