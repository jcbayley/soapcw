...
class MakeSFTs(object):
    ...
    def get_spectrogram(
        ...
        self.strain, self.gpsstart, self.ts = self.read_frame(
            frame_filename, channel_name
        )
        self.fs = 1.0 / self.ts
        # create spectrogram
        spect = self.make_sft(
    ...
    def sfts_from_timeseries(
        ...
        if real:
            ...
            fft_freqs = np.zeros(
                (len(time_segments), len(time_segments[0]))
            )  # /2 + 1))
        elif not real:
            ffts = np.zeros(
                (len(time_segments), len(time_segments[0]) / 2), dtype=np.complex_
            )
            fft_freqs = np.zeros(
                (len(time_segments), len(time_segments[0]))
            )  # /2 + 1))
