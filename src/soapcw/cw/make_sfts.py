...
class MakeSFTs(object):
    ...
    def get_spectrogram(
        self, frame_filename, channel_name, fft_length, overlap=None, fmin=None, fmax=None
    ):
        """
        ...
        # create spectrogram
        spect = self.make_sft(
            self.strain, self.fs, fft_length, overlap=overlap, fmin=fmin, fmax=fmax
        )
    ...
    def make_sft_from_gwf(
        ...
        # create ffts
        fft_freqs, ffts = self.sfts_from_timeseries(
            self.strain, self.fs, fft_length, window=window, fmin=fmin_index, fmax=fmax_index
        )

        # write ffts
        self.sft_file_from_array(
            output_path, ffts, self.fs, fft_length, detector_name, self.gpsStart, fmin, fmax
        )
    ...
    def sfts_from_timeseries(
        self, timeseries, fs, fft_length, window=None, fmin=None, fmax=None, real=True
    ):
        """
        ...
        for i in range(segment_number):
            time_segments.append(
                np.array(timeseries[i * fft_length_index : fft_length_index * (i + 1)])
            )
        ...
        for t, seg in enumerate(time_segments):
            ...
            if real:
                ...
                fft_freqs[t] = np.fft.rfftfreq(fft_length_index, 1.0 / fs)
            else:
                fft_calc = np.fft.fft(seg_wind)
                ffts[t] = fft_calc[: int(len(fft_calc) / 2)]
                fft_freqs[t] = np.fft.fftfreq(fft_length_index, 1.0 / fs)

        fft_freqs_slice = fft_freqs[:, fmin_index:fmax_index]
        ffts_slice = ffts[:, fmin_index:fmax_index]
        return fft_freqs_slice, ffts_slice

    def sft_file_from_array(
        self, output_path, ffts, fs, fft_length, detector_name, gpsStart, fmin, fmax
    ):
        """
        ...
        for t, fft in enumerate(ffts):
            sft_temp = lal.CreateCOMPLEX8FrequencySeries(
                detector_name,
                lal.LIGOTimeGPS(gpsStart),
                fmin,
                1.0 / fft_length,
                lal.SecondUnit,
                int(fft_length_index / 2.0),
            )

            sft_temp.data.data[:] = fft
            filename_prefix = "{}/{}_DT{}_T{}_F{}_{}.sft".format(
                output_path, detector_name, fft_length, gpsStart + t * fft_length, fmin, fmax
            )
