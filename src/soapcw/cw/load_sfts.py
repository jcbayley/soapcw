...
class LoadSFT:
    def __init__(
        ...
        if type(sftpath) is str:
            ...
            raise Exception(
                f"Please use a type str or np.ndarray for the input of sftpath, you used type {type(sftpath)}"
            )
        ...
        if norm is not None:
            if norm == True:
                ...
                norm = 100
                self.norm_rngmed(
                    med_width=norm, remove_sft=remove_sft, save_rngmed=save_rngmed
                )
            elif norm == False:
                pass
            else:
    ...
    def get_sft(
        self, sftpath, fmin=None, fmax=None, tmin=None, tmax=None, vetolist=None
    ):
        """
        ...
        if len(set(nbins)) > 1:
            print(
                "Warning: different detectors do not have the same number of frequency bins"
            )
    ...
    def get_sft_from_array(
        ...
        for name, sdict in sftpath.items():
            ...
            assert len(sdict["sft"]) == len(
                sdict["epochs"]
            ), f'Timestamps (Epochs) should match the length of the SFT, sftlen:{len(sdict["sft"])}, epochlen:{len(sdict["epochs"])}'
            ...
            if "frequencies" in sdict:
                ...
                raise Exception(
                    f"Please define the frequencies or f0 and delta_f in the input dict, currently only {sdict.keys()}"
                )
    ...
    def norm_rngmed(self, med_width=100, remove_sft=False, save_rngmed=False):
        ...
        for det in self.det_names:
            ...
            sig2 = lal.CreateREAL8FrequencySeries(
                None, 0, 0, 1.0 / sfts.tsft, lal.SecondUnit, sfts.nbins
            )
            c_sft = lal.CreateCOMPLEX8FrequencySeries(
                None, 0, 0, 1.0 / sfts.tsft, lal.SecondUnit, sfts.nbins
            )
            ...
            for i, sft in enumerate(sfts.sft):
                ...
                sfts.norm_sft_power[i, :] = (
                    2 * np.abs(c_sft.data.data / np.sqrt(sig2.data.data)) ** 2
                )
        ...
        for det in self.det_names:
            ...
            if hasattr(sft, "sft"):
                if sft.sft is not None:
                    ...
                    new_sft = np.array(
                        np.ones((sft_index[-1] + 1, sft.sft.shape[1])) * 2
                    ).astype("complex")
            ...
            if hasattr(sft, "norm_sft_power"):
                if sft.norm_sft_power is not None:
                    new_sft = (
                        np.ones((sft_index[-1] + 1, sft.norm_sft_power.shape[1])) * 2
                    )
                    for i, idx in enumerate(sft_index):
            ...
            if hasattr(sft, "rng_med"):
                if sft.rng_med is not None:
                    new_sft = (
                        np.ones((sft_index[-1] + 1, sft.rng_med.shape[1])) * np.nan
                    )
                    for i, idx in enumerate(sft_index):
    ...
    def align_sfts(self):
        ...
        for det in self.det_names:
            ...
            if hasattr(sft, "sft"):
                if sft.sft is not None:
                    new_sft = np.array(
                        np.ones((num_ep + 1, sft.sft.shape[1])) * 2
                    ).astype("complex")
                    for i, idx in enumerate(sft_index):
                        new_sft[idx, :] = sft.sft[i, :]
                    sft.sft = new_sft
