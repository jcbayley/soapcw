...
class CVAE(nn.Module):
    def __init__(
        ...
        # encoder r1(z|y)
        self.rencoder_conv, self.rencoder_lin, _ctr = self.create_network(
            "r",
            self.input_dim,
            self.latent_dim,
            append_dim=self.fdim,
        ...
        # encoder q(z|x, y)
        qencoder_conv, self.qencoder_lin, self.conv_track_network = self.create_network(
            "q",
            self.input_dim,
            self.latent_dim,
            append_dim=self.par_dim + self.fdim,
            fc_layers=self.fc_layers,
            conv_layers=self.conv_layers,
            track_conv=True,
        )
        ...
        # decoder r2(x|z, y)
        decoder_conv, self.decoder_lin, _ctr = self.create_network(
            "d",
            self.input_dim,
            self.par_dim,
            append_dim=self.latent_dim + self.fdim,
            fc_layers=self.fc_layers,
            conv_layers=self.conv_layers,
    ...
    def create_network(
        ...
        for i in range(num_conv):
            padding = int(conv_layers[i][1] / 2.0)  # padding half width
            maxpool = nn.MaxPool1d(
                conv_layers[i][3]
            )  # define max pooling for this layer
            # add convolutional/activation/maxpooling layers
            conv_network.add_module(
                "r_conv{}".format(i),
                module=nn.Conv1d(
                    inchannels,
                    conv_layers[i][0],
            ...
            outsize = int(
                self.conv_out_size(
                    insize, padding, conv_layers[i][2], conv_layers[i][1], self.stride
                )
                / conv_layers[i][3]
        ...
        if track_conv:
            ...
            for i in range(num_conv_track):
                padding = int(conv_layers_track[i][1] / 2.0)  # padding half width
                maxpool_track = nn.MaxPool1d(
                    conv_layers_track[i][3]
                )  # define max pooling for this layer
                # add convolutional/activation/maxpooling layers
                conv_track_network.add_module(
                    "r_conv{}".format(i),
                    module=nn.Conv1d(
                        inchannels_track,
                        conv_layers_track[i][0],
                        conv_layers_track[i][1],
                        stride=self.stride,
                        padding=padding,
                        dilation=conv_layers_track[i][2],
                    ),
                )
                conv_track_network.add_module(
                    "act_r_conv{}".format(i), module=self.activation
                )
                # conv_network.add_module("batch_norm_conv{}".format(i), module = nn.BatchNorm1d(conv_layers[i][0]))
                conv_track_network.add_module(
                    "pool_r_conv{}".format(i), module=maxpool_track
                )
                # define the output size of the layer
                outsize_track = int(
                    self.conv_out_size(
        ...
        lin_input_size = (
            np.prod(layer_out_sizes[-1]) if num_conv > 0 else self.input_dim
        )
        if append_dim:
        ...
        for i in range(num_fc):
            lin_network.add_module(
                "r_lin{}".format(i), module=nn.Linear(layer_size, fc_layers[i])
            )
        ...
        if variance:
            setattr(
                self, "log_var_{}".format(name[0]), nn.Linear(layer_size, output_dim)
            )
    ...
    def encode_r(self, y, freqs):
        ...
        conv = (
            self.rencoder_conv(
                torch.reshape(y, (y.size(0), self.inchannels, self.input_dim))
            )
            if self.num_conv > 0
            else y
        )
    ...
    def encode_q(self, y, par, freqs):
        ...
        conv_track = self.conv_track_network(
            torch.reshape(par[:, 4:], (par.size(0), 1, self.input_dim))
        )
    ...
    def KL_gauss(self, mu_r, log_var_r, mu_q, log_var_q):
        ...
        t3 = (torch.square(mu_q - mu_r) + torch.square(sigma_q)) / (
            2 * torch.square(sigma_r)
        )
    ...
    def forward(self, y, par, freqs):
        ...
        if self.dist_type == "studentT":
            mu_par, log_var_par, weight_par = self.decode(
                z_sample, y, freqs
            )  # decode r2(x|z, y)
    ...
    def compute_loss(self, y, par, freqs, ramp):
        ...
        gauss_loss = self.log_likelihood_trunc_gauss(
            par[:, :4], mu_par[:, :4], log_var_par, ramp
        )

        cat_loss = self.log_likelihood_cat(par[:, 4:], mu_par[:, 4:])

        recon_loss = -1.0 * torch.mean(
            torch.sum(torch.cat([gauss_loss, cat_loss], axis=1), axis=1)
        )
    ...
    def draw_samples(
        ...
        ys = y.repeat(num_samples, 1, 1).view(
            -1, y.size(0), y.size(1)
        )  # repeat data so same size as the z samples
        freqs = freqs.repeat(num_samples).view(
            -1
        )  # repeat data so same size as the z samples

        # sample parameter space from returned mean and variance
        mu_par, log_var_par = self.decode(
            z_sample, ys, freqs
        )  # decode r2(x|z, y) from z
        samp = self.trunc_gauss_sample(mu_par[:, :4], log_var_par[:, :4])

        if np.any(samp[:, :3].cpu().numpy() > 1) or np.any(
            samp.cpu().numpy()[:, :3] < 0
        ):
    ...
    def test_latent(self, y, freqs, par, num_samples):
        ...
        for i in range(num_data):
            ...
            zr_sample = self.gauss_sample(
                mu_r[i], log_var_r[i], num_samples, self.latent_dim
            )
            zq_sample = self.gauss_sample(
                mu_q[i], log_var_q[i], num_samples, self.latent_dim
            )
            zr_samples.append(zr_sample.cpu().numpy())
            zq_samples.append(zq_sample.cpu().numpy())
            # input the latent space samples into decoder r2(x|z, y)
            ys = (
                y[i].repeat(num_samples, 1, 1).view(-1, y.size(1), y.size(2))
            )  # repeat data so same size as the z samples
            freqss = (
                freqs[i].repeat(num_samples).view(-1)
            )  # repeat data so same size as the z samples
    ...
    def test(
        self, y, freqs, num_samples, transform_func=None, return_latent=False, par=None
    ):
        ...
        for i in range(num_data):
            ...
            t_net_samples, t_znet_samples, t_track_samples = self.draw_samples(
                y[i],
                mu_r[i],
                log_var_r[i],
                num_samples,
            ...
            if transform_func is None:
                ...
                if np.any(np.isnan(t_net_samples)):
                    ...
                    stime = time.time()
                    while num_nans > 0:
                        nan_locations = np.where(
                            np.any(np.isnan(t_net_samples), axis=1)
                        )
        ...
        if return_latent:
            return (
                np.array(transformed_samples),
                np.array(net_samples),
                np.array(z_samples),
                np.array(q_samples),
                np.array(track_samples),
            )
        else:
            return (
                np.array(transformed_samples),
