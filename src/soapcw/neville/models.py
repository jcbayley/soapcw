...
class CVAE(nn.Module):
    ...
    def encode_r(self, y, freqs):
        ...
        conv = (
            self.rencoder_conv(torch.reshape(y, (y.size(0), self.inchannels, self.input_dim)))
            if self.num_conv > 0
            else y
        )
    ...
    def draw_samples(
        self, y, mu_r, log_var_r, num_samples, latent_dim, par_dim, return_latent=False, freqs=None
    ):
    ...
    def test(self, y, freqs, num_samples, transform_func=None, return_latent=False, par=None):
        ...
        for i in range(num_data):
            ...
            if return_latent:
                q_samples.append(
                    self.gauss_sample(mu_q[i], log_var_q[i], num_samples, self.latent_dim)
                    .cpu()
                    .numpy()
                )
