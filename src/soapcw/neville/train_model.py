...
from tools import loss_plot, make_ppplot, track_plot
    ...
    for epoch in range(epochs):
        ...
        if epoch > dec_start:
            adjust_learning_rate(
                learning_rate, optimizer, epoch - dec_start, factor=dec_rate, low_cut=low_cut
            )
        ...
        for ind in range(len(train_iterator)):
            ...
            local_batch, local_labels, local_freqs = (
                torch.Tensor(local_batch).to(device),
                torch.Tensor(local_labels).to(device),
                torch.Tensor(local_freqs).to(device),
            )
            start_time = time.time()
            train_loss, kl_loss, lik_loss = train_batch(
                epoch,
                model,
                optimizer,
                device,
        ...
        for ind in range(len(validation_iterator)):
            ...
            val_batch, val_labels, val_freqs = (
                torch.Tensor(val_batch).to(device),
                torch.Tensor(val_labels).to(device),
                torch.Tensor(val_freqs).to(device),
            )
            val_loss, val_kl_loss, val_lik_loss = train_batch(
                epoch,
                model,
                optimizer,
                device,
        ...
        if epochs - epoch < 10:
            torch.save(
                model, os.path.join(save_dir, "model_epoch{}.pt".format(epoch))
            )  # save the model

        if epoch % 40 == 0:
            print(
                "Train:      Epoch: {}, Training loss: {}, kl_loss: {}, l_loss:{}, Epoch time: {}, batch time: {}".format(
                    epoch, temp_train_loss, temp_kl_loss, temp_lik_loss, total_time, batch_time
                )
            )
            print(
                "Validation: Epoch: {}, Training loss: {}, kl_loss: {}, l_loss:{}".format(
                    epoch, temp_val_loss, temp_val_kl_loss, temp_val_lik_loss
                )
            )
            loss_plot(
                save_dir,
        ...
        if epoch % int(epochs / 20) == 0:  # and epoch > 3000:
            if do_test or epoch == epochs - 1:
                # test plots
                samples, tr, zr_sample, zq_sample = run_latent(
                    model, validation_iterator, num_samples=1000, device=device
                )
                ...
                if test_data is not None:
                    test_model(
                        os.path.join(save_dir, "epochs_{}".format(epoch)),
                        test_data,
                        20,
                        copy.copy(model),
                        length=length,
                    ...
                    torch.save(
                        model, os.path.join(save_dir, "epochs_{}".format(epoch), "model.pt")
                    )  # save the model
                    print("done_test")
