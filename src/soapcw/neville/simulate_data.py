...
class CW:
    ...
    def transform_ecliptic_amp(self, fmin, width, f0, fdot, lon, lat):
        ...
        """
        A = f0 * self.r0 * self.omega0 * np.sin(lat) / self.c * 1.0 / self.max_A
        s = (f0 - fmin) / width
        norm_fdot = fdot / (1e-9)
...
class LoadCW(CW):
        ...
        if self.data_type == "cvae":
            ...
            if self.chunk_load:
                return (
                    self.data[index * self.batch_size : (index + 1) * self.batch_size],
                    self.labels[index * self.batch_size : (index + 1) * self.batch_size],
                    self.freqs[index * self.batch_size : (index + 1) * self.batch_size],
                )
            else:
    ...
    def load_new_chunk(self):
        if self.chunk_size * self.batch_size > len(self.all_data):
            ...
            st_ind, en_ind = (
                self.chunk_iter * self.chunk_size * self.batch_size,
                (self.chunk_iter + 1) * self.chunk_size * self.batch_size,
            )
    ...
    def gen_train_data(self, numdata):
        ...
        self.all_file_split = [
            int(fname.split("/")[-1].split("_")[-1].strip(".pkl")) for fname in self.filenames
        ]
        ...
        if not self.chunk_load:
            self.data, self.labels, self.widths, self.parameters = self.load_files(
                self.filenames[:num_filenames], temp_indices_split
            )
            ...
            self.all_data, self.all_labels, self.all_widths, self.all_parameters = self.load_files(
                self.filenames[:num_filenames], temp_indices_split
            )
    ...
    def get_filenames(self):
        ...
        for load_dir in self.load_dirs:
            for fr_ind in range(len(self.frange_low)):
                load_sub_dir = os.path.join(
                    load_dir,
                    "paths",
                    "band_{}_{}".format(self.frange_low[fr_ind], self.frange_high[fr_ind]),
                    "snr_{}_{}".format(self.snrrange_low, self.snrrange_high),
                )
    ...
    def load_files(self, filenames, temp_indices):
        ...
        for i, filename in enumerate(filenames):
            ...
            for index, pars in enumerate(params):
                ...
                pars["lon"], pars["lat"] = self.transform_geo_ecliptic(
                    copy.copy(pars["alpha"]), copy.copy(pars["delta"])
                )
                # take abs as only northern hemisphere
                pars["lat"] = np.abs(pars["lat"])
                # reconvert the sky position back to geocentric
                pars["alpha"], pars["delta"] = self.transform_ecliptic_geo(
                    copy.copy(pars["lon"]), copy.copy(pars["lat"])
                )
                ...
                off, A, ph, norm_fdot = self.transform_ecliptic_amp(
                    copy.copy(pars["fmin"]),
                    copy.copy(pars["width"]),
                    copy.copy(pars["f"]),
                    copy.copy(pars["fd"]),
                    copy.copy(pars["lon"]),
                    copy.copy(pars["lat"]),
                ...
                tr_lon, tr_lat, off, tr_fdot = self.transform_lonlat_normlonlat(
                    copy.copy(pars["fmin"]),
                    copy.copy(pars["width"]),
                    copy.copy(pars["lon"]),
                    copy.copy(pars["lat"]),
            ...
            temp_save_pars = np.reshape(
                np.array(save_pars)[get_index], (temp_tracks.shape[0], num_pars)
            )
            ...
            for index in range(len(temp_tracks)):
                ...
                temp_track = (
                    temp_tracks[index] * temp_params[index]["width"] + temp_params[index]["fmin"]
                )
                abstrackdiff = np.array(np.abs(trackinj - temp_track) * 1800 < 2).astype(int)
                temp_loc_true.append(abstrackdiff)

            temp_loc_true = np.reshape(
                np.array(temp_loc_true), (temp_tracks.shape[0], temp_tracks.shape[1])
            )
            ...
            if self.load_powers:
                temp_data = np.reshape(
                    np.array(
                        [
                            np.array([temp_tracks[ti], powers1[ti], powers2[ti]])
                            for ti in range(len(temp_tracks))
                        ]
                    ),
                    (temp_tracks.shape[0], 3, temp_tracks.shape[1]),
                )
            else:
